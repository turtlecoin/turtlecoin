...
struct TransactionInput
{
    ...
    bool operator==(const TransactionInput &other) { return keyImage == other.keyImage; }
...
{
    TxInputAndOwner(const TransactionInput input, const Crypto::PublicKey publicSpendKey,
...
class Transaction
{
  public:
    ...
    Transaction(
        ...
        const std::unordered_map<Crypto::PublicKey, int64_t> transfers, const Crypto::Hash hash, const uint64_t fee,
        const uint64_t timestamp, const uint64_t blockHeight, const std::string paymentID, const uint64_t unlockTime,
        const bool isCoinbaseTransaction)
        :

          transfers(transfers), hash(hash), fee(fee), timestamp(timestamp), blockHeight(blockHeight),
          paymentID(paymentID), unlockTime(unlockTime), isCoinbaseTransaction(isCoinbaseTransaction)
    {
    }
    ...
    bool isFusionTransaction() const { return fee == 0 && !isCoinbaseTransaction; }
...
{
    j = {{"outputs", r.keyOutputs},    {"hash", r.hash},           {"txPublicKey", r.transactionPublicKey},
         {"unlockTime", r.unlockTime}, {"paymentID", r.paymentID}, {"inputs", r.keyInputs}};
}
...
inline void to_json(nlohmann::json &j, const KeyOutput &k) { j = {{"key", k.key}, {"amount", k.amount}}; }
