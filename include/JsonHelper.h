...
typedef rapidjson::GenericObject<
    true, rapidjson::GenericValue<rapidjson::UTF8<char>, rapidjson::MemoryPoolAllocator<rapidjson::CrtAllocator>>>
    JSONObject;
...
template <typename T> uint64_t getUint64FromJSON(const T &j, const std::string &key)
{
    ...
    if (!val.IsUint64())
    {
        throw std::invalid_argument("JSON parameter is wrong type. Expected uint64_t, got " +
