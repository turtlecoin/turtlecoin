...
_configure_ubuntu() {
    ...
    $_sudo apt-get install -qq -y git build-essential python-dev gcc g++ git cmake libboost-all-dev libreadline-dev >>build.log 2>&1 || _fail "Unable to install build dependencies. Please see build.log for more information"
