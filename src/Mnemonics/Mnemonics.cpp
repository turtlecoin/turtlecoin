...
std::tuple<Error, Crypto::SecretKey> MnemonicToPrivateKey(const std::vector<std::string> words)
{
    ...
    if (len != 25)
    {
        ...
        Error error(MNEMONIC_WRONG_LENGTH, "The mnemonic seed given is the wrong length. It should be "
    ...
    for (auto word : words)
    {
        ...
        {
            Error error(MNEMONIC_INVALID_WORD, "The mnemonic seed given has a word that is not present "
