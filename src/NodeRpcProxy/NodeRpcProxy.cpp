...
NodeRpcProxy::NodeRpcProxy(const std::string &nodeHost, unsigned short nodePort, unsigned int initTimeout,
    ...
    : m_logger(logger, "NodeRpcProxy"), m_rpcTimeout(10000), m_pullInterval(5000), m_nodeHost(nodeHost),
      m_nodePort(nodePort), m_initTimeout(initTimeout), m_connected(true), m_peerCount(0), m_networkHeight(0),
      m_nodeHeight(0)
{
    resetInternalState();
}
...
NodeRpcProxy::NodeRpcProxy(const std::string &nodeHost, unsigned short nodePort, unsigned int initTimeout)
    : m_logger(std::make_shared<Logging::DummyLogger>(), "NodeRpcProxy"), m_rpcTimeout(10000), m_pullInterval(5000),
      m_nodeHost(nodeHost), m_nodePort(nodePort), m_initTimeout(initTimeout), m_connected(true), m_peerCount(0),
      m_networkHeight(0), m_nodeHeight(0)
{
    resetInternalState();
}
...
uint32_t NodeRpcProxy::getKnownBlockCount() const { return m_networkHeight.load(std::memory_order_relaxed) + 1; }

uint64_t NodeRpcProxy::getNodeHeight() const { return m_nodeHeight.load(std::memory_order_relaxed); }
                                              ...
                                              std::vector<Crypto::Hash> &blockHashes, const Callback &callback)
{
    ...
    scheduleRequest(std::bind(&NodeRpcProxy::doGetBlockHashesByTimestamps, this, timestampBegin, secondsCount,
                              std::ref(blockHashes)),
                    callback);
}
...
void NodeRpcProxy::getRandomOutsByAmounts(std::vector<uint64_t> &&amounts, uint16_t outsCount,
                                          std::vector<RandomOuts> &outs, const Callback &callback)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_state != STATE_INITIALIZED)
    {
...
void NodeRpcProxy::getGlobalIndexesForRange(const uint64_t startHeight, const uint64_t endHeight,
...
void NodeRpcProxy::getTransactionsStatus(const std::unordered_set<Crypto::Hash> transactionHashes,
                                         ...
                                         const Callback &callback)
{
    ...
    scheduleRequest(std::bind(&NodeRpcProxy::doGetTransactionsStatus, this, transactionHashes,
...
void NodeRpcProxy::getWalletSyncData(std::vector<Crypto::Hash> &&knownBlockIds, uint64_t startHeight,
                                     uint64_t startTimestamp, std::vector<WalletTypes::WalletBlockInfo> &newBlocks,
                                     const Callback &callback)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_state != STATE_INITIALIZED)
    ...
    scheduleRequest(std::bind(&NodeRpcProxy::doGetWalletSyncData, this, std::move(knownBlockIds), startHeight,
                              startTimestamp, std::ref(newBlocks)),
                    callback);
}
...
void NodeRpcProxy::getBlocks(const std::vector<uint32_t> &blockHeights, std::vector<std::vector<BlockDetails>> &blocks,
                             const Callback &callback)
{
    std::lock_guard<std::mutex> lock(m_mutex);
    if (m_state != STATE_INITIALIZED)
...
std::error_code
NodeRpcProxy::doGetGlobalIndexesForRange(const uint64_t startHeight, const uint64_t endHeight,
                                         std::unordered_map<Crypto::Hash, std::vector<uint64_t>> &indexes)
{
...
std::error_code NodeRpcProxy::doGetTransactionsStatus(const std::unordered_set<Crypto::Hash> transactionHashes,
                                                      std::unordered_set<Crypto::Hash> &transactionsInPool,
                                                      std::unordered_set<Crypto::Hash> &transactionsInBlock,
                                                      std::unordered_set<Crypto::Hash> &transactionsUnknown)
{
...
std::error_code NodeRpcProxy::doGetWalletSyncData(const std::vector<Crypto::Hash> &knownBlockIds, uint64_t startHeight,
...
std::error_code NodeRpcProxy::doGetBlocksByHeight(const std::vector<uint32_t> &blockHeights,
