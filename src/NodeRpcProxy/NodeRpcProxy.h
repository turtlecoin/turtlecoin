...
class NodeRpcProxy : public CryptoNote::INode
{
  public:
    ...
    virtual void getRandomOutsByAmounts(std::vector<uint64_t> &&amounts, uint16_t outsCount,
                                        std::vector<RandomOuts> &result, const Callback &callback) override;
    virtual void getTransactionOutsGlobalIndices(const Crypto::Hash &transactionHash,
                                                 std::vector<uint32_t> &outsGlobalIndices,
                                                 const Callback &callback) override;

    virtual void getGlobalIndexesForRange(const uint64_t startHeight, const uint64_t endHeight,
    ...
    virtual void queryBlocks(std::vector<Crypto::Hash> &&knownBlockIds, uint64_t timestamp,
                             ...
                             const Callback &callback) override;
    virtual void getWalletSyncData(std::vector<Crypto::Hash> &&knownBlockIds, uint64_t startHeight,
  ...
  private:
    ...
    std::error_code doGetGlobalIndexesForRange(const uint64_t startHeight, const uint64_t endHeight,
                                               std::unordered_map<Crypto::Hash, std::vector<uint64_t>> &indexes);

    std::error_code doGetTransactionsStatus(const std::unordered_set<Crypto::Hash> transactionHashes,
                                            std::unordered_set<Crypto::Hash> &transactionsInPool,
                                            std::unordered_set<Crypto::Hash> &transactionsInBlock,
                                            std::unordered_set<Crypto::Hash> &transactionsUnknown);
