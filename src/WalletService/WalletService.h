...
class WalletService
{
  public:
    ...
    std::error_code createAddress(const std::string &spendSecretKeyText, const uint64_t scanHeight,
                                  const bool newAddress, std::string &address);
    ...
    std::error_code getBlockHashes(uint32_t firstBlockIndex, uint32_t blockCount,
