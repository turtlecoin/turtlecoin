...
namespace CryptoNote
{
    ...
    private:
        ...
        bool new_wallet(Crypto::SecretKey &secret_key, Crypto::SecretKey &view_key, const std::string &wallet_file,
        ...
        public:
            refresh_progress_reporter_t(CryptoNote::pool_wallet &pool_wallet)
                    : m_pool_wallet(pool_wallet), m_blockchain_height(0), m_blockchain_height_update_time(),
                      m_print_time()
            {
            }
            ...
            void update(uint64_t height, bool force = false)
            {
                ...
                if (std::chrono::seconds(m_pool_wallet.currency().difficultyTarget() / 2) <
