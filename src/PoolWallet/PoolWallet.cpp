...
namespace
{
    ...
    const command_line::arg_descriptor<std::string> arg_generate_new_wallet = {"generate-new-wallet",
                                                                               "Generate new wallet and save it to <arg>",
                                                                               ""};
    const command_line::arg_descriptor<std::string> arg_daemon_address = {"daemon-address",
                                                                          "Use daemon instance at <host>:<port>", ""};
    const command_line::arg_descriptor<std::string> arg_daemon_host = {"daemon-host",
                                                                       "Use daemon instance at host <arg> instead of localhost",
                                                                       ""};
    const command_line::arg_descriptor<std::string> arg_password = {"password", "Wallet password", "", true};
    const command_line::arg_descriptor<uint16_t> arg_daemon_port = {"daemon-port",
                                                                    "Use daemon instance at port <arg> instead of 11898",
                                                                    0};
    const command_line::arg_descriptor<uint32_t> arg_log_level = {"set_log", "", INFO, true};
    const command_line::arg_descriptor<bool> arg_SYNC_FROM_ZERO = {"SYNC_FROM_ZERO",
                                                                   "Sync from block 0. Use for premine wallet or brainwallet",
                                                                   false};
    const command_line::arg_descriptor<bool> arg_exit_after_generate = {"exit-after-generate",
                                                                        "Exit immediately after generating a wallet, do not try to sync with the daemon",
                                                                        false};
    const command_line::arg_descriptor<bool> arg_testnet = {"testnet",
                                                            "Used to deploy test nets. The daemon must be launched with --testnet flag",
                                                            false};
    const command_line::arg_descriptor<std::vector<std::string> > arg_command = {"command", ""};
    const command_line::arg_descriptor<std::string> arg_restore_view = {"restore-view-key",
                                                                        "Specify the View Key to re-generate an existing wallet",
                                                                        ""};
    const command_line::arg_descriptor<std::string> arg_restore_spend = {"restore-spend-key",
                                                                         "Specify the Spend Key to re-generate an existing wallet",
                                                                         ""};
    ...
    std::string tryToOpenWalletOrLoadKeysOrThrow(LoggerRef &logger, std::unique_ptr<IWalletLegacy> &wallet,
                                                 const std::string &walletFile, const std::string &password)
    {
    ...
    void printListTransfersItem(LoggerRef &logger, const WalletLegacyTransaction &txInfo, IWalletLegacy &wallet,
                                const Currency &currency, uint64_t currentHeight)
    {
        std::vector<uint8_t> extraVec = Common::asBinaryArray(txInfo.extra);

        Crypto::Hash paymentId;
        std::string paymentIdStr = (getPaymentIdFromTxExtra(extraVec, paymentId) && paymentId != NULL_HASH
        ...
        if (txInfo.totalAmount < 0)
        {
            if (txInfo.transferCount > 0)
            {
                ...
                for (TransferId id = txInfo.firstTransferId; id < txInfo.firstTransferId + txInfo.transferCount; ++id)
                {
                    ...
                    logger(INFO, rowColor) << tr.address << "  " << std::setw(TOTAL_AMOUNT_MAX_WIDTH)
                                           << currency.formatAmount(tr.amount);
                }
            }
        }
...
pool_wallet::pool_wallet(System::Dispatcher &dispatcher, const CryptoNote::Currency &currency,
        ...
        m_walletSynchronized(false)
{
    ...
    m_consoleHandler.setHandler("export_keys", boost::bind(&pool_wallet::export_keys, this, _1),
                                "Show the secret keys of the opened wallet");
    m_consoleHandler.setHandler("balance", boost::bind(&pool_wallet::show_balance, this, _1),
                                "Show current wallet balance");
    m_consoleHandler.setHandler("incoming_transfers", boost::bind(&pool_wallet::show_incoming_transfers, this, _1),
                                "Show incoming transfers");
    m_consoleHandler.setHandler("outgoing_transfers", boost::bind(&pool_wallet::show_outgoing_transfers, this, _1),
                                "Show outgoing transfers");
    m_consoleHandler.setHandler("list_transfers", boost::bind(&pool_wallet::listTransfers, this, _1),
                                "Show all known transfers");
    m_consoleHandler.setHandler("payments", boost::bind(&pool_wallet::show_payments, this, _1),
                                "payments <payment_id_1> [<payment_id_2> ... <payment_id_N>] - Show payments <payment_id_1>, ... <payment_id_N>");
    m_consoleHandler.setHandler("bc_height", boost::bind(&pool_wallet::show_blockchain_height, this, _1),
    ...
    m_consoleHandler.setHandler("set_log", boost::bind(&pool_wallet::set_log, this, _1),
                                "set_log <level> - Change current log level, <level> is a number 0-4");
    m_consoleHandler.setHandler("address", boost::bind(&pool_wallet::print_address, this, _1),
                                "Show current wallet public address");
    m_consoleHandler.setHandler("view_tx_outputs", boost::bind(&pool_wallet::print_outputs_from_transaction, this, _1),
                                "view_tx_outputs <transaction_hash> - Find outputs that belong to you in a transaction");
    m_consoleHandler.setHandler("save", boost::bind(&pool_wallet::save, this, _1), "Save wallet synchronized data");
    m_consoleHandler.setHandler("reset", boost::bind(&pool_wallet::reset, this, _1),
...
bool pool_wallet::init(const boost::program_options::variables_map &vm)
{
    ...
    } else if (!m_import_new.empty())
    {
        ...
        if (key_import)
        {
            ...
            size_t size;
            if (!Common::fromHex(private_spend_key_string, &private_spend_key_hash, sizeof(private_spend_key_hash),
                                 size) || size != sizeof(private_spend_key_hash))
            {
                return false;
            }
            if (!Common::fromHex(private_view_key_string, &private_view_key_hash, sizeof(private_view_key_hash),
                                 size) || size != sizeof(private_spend_key_hash))
            {
                return false;
            }
    ...
    } else
    {
        ...
        try
        {
            m_wallet_file = tryToOpenWalletOrLoadKeysOrThrow(logger, m_wallet, m_wallet_file_arg,
                                                             m_pwd_container.password());
...
//----------------------------------------------------------------------------------------------------
bool pool_wallet::new_wallet(Crypto::SecretKey &secret_key, Crypto::SecretKey &view_key, const std::string &wallet_file,
...
bool pool_wallet::show_balance(const std::vector<std::string> &args/* = std::vector<std::string>()*/)
{
    success_msg_writer() << "available balance: " << m_currency.formatAmount(m_wallet->actualBalance()) <<
                         ...
                         ", total amount: "
                         << m_currency.formatAmount(m_wallet->actualBalance() + m_wallet->pendingBalance());
    ...
    for (const std::string &arg: args)
    {
        ...
        if (CryptoNote::parsePaymentId(arg, expectedPaymentId))
        {
            ...
            for (size_t trantransactionNumber = 0; trantransactionNumber < transactionsCount; ++trantransactionNumber)
            {
                ...
                std::for_each(txInfo.extra.begin(), txInfo.extra.end(), [&extraVec](const char el)
...
bool pool_wallet::transfer(const std::vector<std::string> &args)
{
    try
    {
        ...
        CryptoNote::TransactionId tx = m_wallet->sendTransaction(cmd.dsts, cmd.fee, extraString, cmd.fake_outs_count,
                                                                 0);
...
bool pool_wallet::print_outputs_from_transaction(const std::vector<std::string> &args)
{
    ...
    {
        logger(ERROR, BRIGHT_RED)
                << "Failed to find transaction hash! Ensure you entered it correctly and your daemon is fully synced.";
        return false;
    }
    ...
    if (!found)
    {
        logger(ERROR, BRIGHT_RED) << "No outputs were found that belong to you, searched "
                                  << ourTransaction.outputs.size() << " outputs.";
    } else
    {
...
int main(int argc, char *argv[])
{
    ...
    bool r = command_line::handle_error_helper(desc_all, [&]()
    {
        ...
        if (command_line::get_arg(vm, command_line::arg_help))
        {
            ...
            std::cout
                    << "Usage: poolwallet [--wallet-file=<file>|--generate-new-wallet=<file>] [--daemon-address=<host>:<port>] [<COMMAND>]";
