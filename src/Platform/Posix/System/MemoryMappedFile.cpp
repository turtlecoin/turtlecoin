...
namespace System
{
    ...
    void MemoryMappedFile::create(const std::string &path, uint64_t size, bool overwrite, std::error_code &ec)
    {
        ...
        m_data = reinterpret_cast<uint8_t *>(::mmap(nullptr, static_cast<size_t>(size), PROT_READ | PROT_WRITE,
                                                    MAP_SHARED, m_file, 0));
    ...
    void MemoryMappedFile::open(const std::string &path, std::error_code &ec)
    {
        ...
        m_data = reinterpret_cast<uint8_t *>(::mmap(nullptr, static_cast<size_t>(m_size), PROT_READ | PROT_WRITE,
                                                    MAP_SHARED, m_file, 0));
        if (m_data == MAP_FAILED)
        {
            return;
    ...
    void MemoryMappedFile::flush(uint8_t *data, uint64_t size, std::error_code &ec)
    {
        ...
        int result = ::msync(reinterpret_cast<void *>(pageOffset), static_cast<size_t>(dataAddr % pageSize + size),
                             MS_SYNC);
        if (result == 0)
        {
            result = ::fsync(m_file);
