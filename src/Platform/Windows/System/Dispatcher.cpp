...
namespace System
{
    ...
    Dispatcher::Dispatcher()
    {
        ...
        BOOL result = InitializeCriticalSectionAndSpinCount(reinterpret_cast<LPCRITICAL_SECTION>(criticalSection),
                                                            4000);
        assert(result != FALSE);
        std::string message;
    ...
    void Dispatcher::dispatch()
    {
        ...
        for (;;)
        {
            ...
            DWORD timeout = timers.empty() ? INFINITE : static_cast<DWORD>(std::min(timers.begin()->first - currentTime,
                                                                                    static_cast<uint64_t>(INFINITE -
                                                                                                          1)));
