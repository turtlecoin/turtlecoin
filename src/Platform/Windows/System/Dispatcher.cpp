...
void Dispatcher::dispatch()
{
    ...
    for (;;)
    {
        ...
        DWORD timeout = timers.empty() ? INFINITE
                                       : static_cast<DWORD>(std::min(timers.begin()->first - currentTime,
        ...
        {
            throw std::runtime_error("Dispatcher::dispatch, GetQueuedCompletionStatusEx failed, " +
...
void Dispatcher::remoteSpawn(std::function<void()> &&procedure)
{
    ...
    if (!remoteNotificationSent)
    {
        ...
        {
            LeaveCriticalSection(reinterpret_cast<LPCRITICAL_SECTION>(criticalSection));
            throw std::runtime_error("Dispatcher::remoteSpawn, PostQueuedCompletionStatus failed, " +
                                     lastErrorMessage());
        };
    }
...
void Dispatcher::yield()
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            {
                throw std::runtime_error("Dispatcher::yield, GetQueuedCompletionStatusEx failed, " +
                                         errorMessage(lastError));
            }
        }
    }
