...
TcpListener::TcpListener(Dispatcher &dispatcher, const Ipv4Address &address, uint16_t port) : dispatcher(&dispatcher)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (acceptEx == nullptr &&
                WSAIoctl(listener, SIO_GET_EXTENSION_FUNCTION_POINTER, &guidAcceptEx, sizeof guidAcceptEx, &acceptEx,
                         sizeof acceptEx, &read, NULL, NULL) != 0)
            {
            ...
            else
            {
                ...
                if (WSAGetOverlappedResult(listener, &context2, &transferred, FALSE, &flags) != TRUE)
                {
                    ...
                    else
                    {
                        ...
                        {
                            throw std::runtime_error("TcpListener::accept, closesocket failed, " +
                ...
                else
                {
                    ...
                    else
                    {
                        if (CreateIoCompletionPort(reinterpret_cast<HANDLE>(connection),
                                                   dispatcher->getCompletionPort(), 0,
