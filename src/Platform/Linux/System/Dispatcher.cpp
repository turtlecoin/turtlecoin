    ...
    for (;;)
    {
        ...
        if (count == 1)
        {
                ...
                contextPair->writeContext == nullptr)
            {
                ...
                if (transferred == -1)
                {
                    throw std::runtime_error("Dispatcher::dispatch, read(remoteSpawnEvent) failed, " +
                                             lastErrorMessage());
                }
...
int Dispatcher::getEpoll() const { return epoll; }
...
NativeContext &Dispatcher::getReusableContext()
{
    if (firstReusableContext == nullptr)
    {
        ...
        makecontext(newlyCreatedContext, (void (*)())contextProcedureStatic, 1,
