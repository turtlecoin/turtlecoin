...
typedef std::vector<
    std::tuple<
        Crypto::PublicKey, WalletTypes::TransactionInput>> BlockInputsAndOwners;

typedef std::tuple<
    WalletTypes::WalletBlockInfo, BlockInputsAndOwners, uint32_t
> SemiProcessedBlock;
...
struct BlockScanTmpInfo
{
    ...
    std::vector<
        std::tuple<
            Crypto::PublicKey, Crypto::KeyImage>> keyImagesToMarkSpent;
};
...
class OrderByArrivalIndex
{
    public:
        ...
        bool operator()(
            SemiProcessedBlock a,
            SemiProcessedBlock b
        )
        {
...
class WalletSynchronizer
{
    ...
    private:
        ...
        std::vector<
            std::tuple<
                Crypto::PublicKey, WalletTypes::TransactionInput>> processBlockOutputs(
        ...
        std::vector<
            std::tuple<
                Crypto::PublicKey, WalletTypes::TransactionInput>> processTransactionOutputs(
            const WalletTypes::RawCoinbaseTransaction &rawTX,
            const uint64_t blockHeight
        ) const;

        std::unordered_map<
            Crypto::Hash, std::vector<uint64_t>> getGlobalIndexes(
            const uint64_t blockHeight
        ) const;
        ...
        ThreadSafeDeque<
            std::tuple<
                WalletTypes::WalletBlockInfo, uint32_t>> m_blockProcessingQueue;
        ...
        ThreadSafePriorityQueue<
            SemiProcessedBlock, OrderByArrivalIndex
        > m_processedBlocks;
