...
namespace SendTransaction
{
    ...
    )
    {
        ...
        Error error = validateFusionTransaction(
            mixin, addressesToTakeFrom, destination, subWallets, daemon->networkBlockCount());
        ...
        const std::vector<Crypto::PublicKey>
            subWalletsToTakeFrom = Utilities::addressesToSpendKeys(addressesToTakeFrom);
        ...
        storeSentTransaction(
            txHash, fee, paymentID, ourInputs, destination, changeRequired, subWallets
        );
    ...
    )
    {
        std::vector<
            std::pair<
                std::string, uint64_t>> destinations = {{destination, amount}};
    ...
    )
    {
        ...
        Error error = validateTransaction(
            addressesAndAmounts, mixin, fee, paymentID, addressesToTakeFrom, changeAddress, subWallets, daemon
            ->networkBlockCount());
        ...
        /* Store the unconfirmed transaction, update our balance */
        storeSentTransaction(
            txHash, fee, paymentID, ourInputs, changeAddress, changeRequired, subWallets
        );
    ...
    )
    {
        ...
        if (txSize > maxTxSize)
        {
            ...
            errorMsg << "Transaction is too large: (" << Utilities::prettyPrintBytes(txSize)
                     ...
                     << ". Decrease the amount you are sending, or perform some " << "fusion transactions.";

            return Error(
                TOO_MANY_INPUTS_TO_FIT_IN_BLOCK, errorMsg.str());
        }
    ...
    )
    {
        ...
        std::transform(
            sources.begin(), sources.end(), std::back_inserter(amounts), [](const auto destination)
        {
        ...
        for (const uint64_t amount : amounts)
        {
            const auto it = std::find_if(
                fakeOuts.begin(), fakeOuts.end(), [amount](const auto output)
            {
            ...
            if (it == fakeOuts.end())
            {
                ...
                return {
                    Error(NOT_ENOUGH_FAKE_OUTPUTS, error.str()),
                    fakeOuts
                };
            }
        ...
        for (auto fakeOut : fakeOuts)
        {
            ...
            std::sort(
                fakeOut.outs.begin(), fakeOut.outs.end(), [](
                const auto &lhs,
                const auto &rhs
            )
            {
        ...
        for (const auto walletAmount : sources)
        {
            ...
            if (mixin != 0)
            {
                ...
                for (const auto fakeOut : fakeOuts[i].outs)
                {
                    ...
                    /* Add the fake output */
                    obscuredInput.outputs.push_back(
                        {
                            fakeOut.global_amount_index,
                            fakeOut.out_key
                        }
                    );
    ...
    )
    {
        ...
        for (const auto input : inputsAndFakes)
        {
            ...
            std::transform(
                input.outputs.begin(), input.outputs.end(), std::back_inserter(
                keyInput.outputIndexes
            ), [](const auto output)
                {
    ...
    )
    {
        ...
        for (const auto input : inputsAndFakes)
        {
            ...
            const auto[success, signatures] = Crypto::crypto_ops::generateRingSignatures(
                txPrefixHash, boost::get<CryptoNote::KeyInput>(tx.inputs[i])
                .keyImage, publicKeys, tmpSecretKeys[i], input.realOutput
            );

            if (!success)
            {
                return {
                    FAILED_TO_CREATE_RING_SIGNATURE,
                    tx
                };
            }
    ...
    bool verifyAmounts(const std::vector<uint64_t> amounts)
    {
        ...
        {
            if (std::find(
                Constants::PRETTY_AMOUNTS.begin(), Constants::PRETTY_AMOUNTS.end(), amount
