...
namespace PaymentService
{
    ...
    void Configuration::initOptions(boost::program_options::options_description &desc)
    {
        ...
        ("unregister-service", "unregister service and exit (Windows only)")
                ...
                ("server-root", po::value<std::string>(),
                ...
                ("enable-cors", po::value<std::string>(),
                 "Adds header 'Access-Control-Allow-Origin' to walletd's RPC responses. Uses the value as domain. Use * for all.");
    }
    ...
    void Configuration::init(const boost::program_options::variables_map &options)
    {
        ...
        if (options.count("log-level") != 0)
        {
            ...
            {
                std::string error = "log-level option must be in " + std::to_string(Logging::FATAL) + ".." +
                                    std::to_string(Logging::TRACE) + " interval";
