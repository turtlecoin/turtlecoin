...
std::shared_ptr<WalletBackend> importViewWallet(const Config &config)
{
    ...
    auto [error, walletBackend] = WalletBackend::importViewWallet(privateViewKey, address, walletFileName, walletPass,
...
std::shared_ptr<WalletBackend> importWalletFromKeys(const Config &config)
{
    ...
    const auto [error, walletBackend] = WalletBackend::importWalletFromKeys(
        privateSpendKey, privateViewKey, walletFileName, walletPass, scanHeight, config.host, config.port);
...
std::shared_ptr<WalletBackend> importWalletFromSeed(const Config &config)
{
    ...
    auto [error, walletBackend] = WalletBackend::importWalletFromSeed(mnemonicSeed, walletFileName, walletPass,
                                                                      scanHeight, config.host, config.port);
...
std::shared_ptr<WalletBackend> createWallet(const Config &config)
{
    ...
    const auto [error, walletBackend] =
        WalletBackend::createWallet(walletFileName, walletPass, config.host, config.port);
...
std::shared_ptr<WalletBackend> openWallet(const Config &config)
{
    ...
    while (true)
    {
        ...
        const auto [error, walletBackend] =
            WalletBackend::openWallet(walletFileName, walletPass, config.host, config.port);
