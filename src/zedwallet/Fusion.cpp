...
size_t makeFusionTransaction(CryptoNote::WalletGreen &wallet, uint64_t threshold, uint64_t height)
{
    ...
    try
    {
        return wallet.createFusionTransaction(bestThreshold, CryptoNote::getDefaultMixinByHeight(height), {},
                                              wallet.getAddress(0));
    }
...
bool optimize(CryptoNote::WalletGreen &wallet, uint64_t threshold, uint64_t height)
{
    ...
    while (true)
    {
        ...
        else
        {
            ...
            {
                std::cout << SuccessMsg("Created " + std::to_string(fusionTransactionHashes.size()) +
                                        " fusion transactions!")
                          << std::endl;
            }
        }
