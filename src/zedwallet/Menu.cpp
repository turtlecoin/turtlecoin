    ...
    while (true)
    {
        ...
        try
        {
            ...
            if (selectionNum < 0 || selectionNum >= numCommands)
            {
                std::cout << WarningMsg("Bad input, expected a command name, ") << WarningMsg("or number from ")
                          << InformationMsg("1") << WarningMsg(" to ") << InformationMsg(std::to_string(numCommands))
                          << std::endl;
        ...
        catch (const std::invalid_argument &)
        {
            ...
            auto it = std::find_if(availableCommands.begin(), availableCommands.end(),
                                   [&selection](const Command &c) { return c.commandName == selection; });
...
void mainLoop(std::shared_ptr<WalletInfo> walletInfo, CryptoNote::INode &node)
{
    ...
    while (true)
    {
        ...
        if (walletInfo->viewWallet)
        {
            command = parseCommand(basicViewWalletCommands(), allViewWalletCommands(), getPrompt(walletInfo), true,
                                   walletInfo);
        }
        else
        {
...
template std::string parseCommand(const std::vector<Command> &printableCommands,
                                  const std::vector<Command> &availableCommands, std::string prompt,
                                  bool backgroundRefresh, std::shared_ptr<WalletInfo> walletInfo);

template std::string parseCommand(const std::vector<AdvancedCommand> &printableCommands,
                                  const std::vector<AdvancedCommand> &availableCommands, std::string prompt,
                                  bool backgroundRefresh, std::shared_ptr<WalletInfo> walletInfo);
