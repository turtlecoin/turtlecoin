...
std::string asString(const void *data, uint64_t size) { return std::string(static_cast<const char *>(data), size); }
...
std::string timeIntervalToString(uint64_t intervalInSeconds)
{
    ...
    std::stringstream ss;
    ss << "d" << days << std::setfill('0') << ".h" << std::setw(2) << hours << ".m" << std::setw(2) << minutes << ".s"
       << std::setw(2) << seconds;
