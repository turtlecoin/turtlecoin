...
uint64_t fromHex(const std::string &text, void *data,
                 uint64_t bufferSize); // Assigns values of hex 'text' to buffer 'data' up to 'bufferSize', returns
                                       // actual data size, throws on error
bool fromHex(const std::string &text, void *data, uint64_t bufferSize,
             uint64_t &size); // Assigns values of hex 'text' to buffer 'data' up to 'bufferSize', assigns actual data
                              // size to 'size', returns false on error, does not throw
...
void toHex(const void *data, uint64_t size,
...
template <class T> std::string podToHex(const T &s) { return toHex(&s, sizeof(s)); }
