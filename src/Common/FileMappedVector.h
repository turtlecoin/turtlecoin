...
void FileMappedVector<T>::open(const std::string& path, FileMappedVectorOpenMode mode, uint64_t prefixSize) {
  ...
  std::filesystem::path bakPath = path + ".bak";
  bool fileExists;
  if (std::filesystem::exists(filePath)) {
    ...
    fileExists = true;
  } else if (std::filesystem::exists(bakPath)) {
    std::filesystem::rename(bakPath, filePath);
    fileExists = true;
  } else {
    fileExists = false;
  }
...
void FileMappedVector<T>::atomicUpdate0(uint64_t newCapacity, uint64_t newPrefixSize, uint64_t newSuffixSize, F&& func) {
  ...
  std::filesystem::remove(bakPath, stdError);
}
