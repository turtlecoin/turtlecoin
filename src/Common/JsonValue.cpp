...
namespace Common {
	...
	JsonValue::JsonValue(Unsigned_Int value)
	{
		type = UNSIGNED_INT;
		value_uint = value;
	}
	...
	uint64_t JsonValue::getUnsigned() const
	{
		if (type != UNSIGNED_INT) {
			throw std::runtime_error("JsonValue type is not UNSIGNED INTEGER");
		}
	...
	void JsonValue::readUnsigned(std::istream & in)
	{
		std::string text;
		//when a number prefixed with '+', nothing happens
		uint64_t dots = 0;
		char c;
		for (;;) {
			int i = in.peek();
