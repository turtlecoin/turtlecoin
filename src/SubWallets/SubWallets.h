...
class SubWallets
{
  public:
    ...
    SubWallets(const Crypto::SecretKey privateSpendKey, const Crypto::SecretKey privateViewKey,
               const std::string address, const uint64_t scanHeight, const bool newWallet);

    /* Creates a new view only subwallet */
    SubWallets(const Crypto::SecretKey privateViewKey, const std::string address, const uint64_t scanHeight,
    ...
    Crypto::KeyImage getTxInputKeyImage(const Crypto::PublicKey publicSpendKey, const Crypto::KeyDerivation derivation,
    ...
    getTransactionInputsForAmount(const uint64_t amount, const bool takeFromAll,
                                  std::vector<Crypto::PublicKey> subWalletsToTakeFrom, const uint64_t height) const;

    std::tuple<std::vector<WalletTypes::TxInputAndOwner>, uint64_t, uint64_t>
    getFusionTransactionInputs(const bool takeFromAll, std::vector<Crypto::PublicKey> subWalletsToTakeFrom,
    ...
    void removeSpentKeyImage(const Crypto::KeyImage keyImage, const Crypto::PublicKey publicKey);
    ...
    void markInputAsSpent(const Crypto::KeyImage keyImage, const Crypto::PublicKey publicKey,
