...
//add <void> specialisation when needed
        ...
        {
        public:
            RemotelySpawnedSyncContext(System::Dispatcher &dispatcher, std::atomic<size_t> &counter,
                                       System::Event &counterEvent, std::function<ReturnType()> &&function) :
                    called(false)
            {
        ...
        {
        public:
            RemoteContextCounterWrapper(System::Dispatcher &dispatcher_, std::function<void()> &&function_,
                                        std::atomic<size_t> &contextCounter_, System::Event &contextCounterEvent_) :
        ...
        void
        remoteSpawn(System::Dispatcher &dispatcher, std::function<void()> &&func, std::atomic<size_t> &contextCounter,
                    System::Event &contextCounterEvent)
        {
            contextCounter++;
    ...
    InProcessNode::InProcessNode(CryptoNote::ICore &core, CryptoNote::ICryptoNoteProtocolHandler &protocol,
            ...
            : state(NOT_INITIALIZED), contextGroup(dispatcher), contextCounter(0), contextCounterEvent(disp),
    ...
    void InProcessNode::init(const Callback &callback)
    {
        ...
        if (state != NOT_INITIALIZED)
        {
            ...
            executeInRemoteThread([callback, ec]
                                  { callback(ec); });
            return;
        }
                                   ...
                                   while (true)
                                   {
                                       messageQueue.front().match(
                                               [this](const NewBlock &msg)
                                               {
                                                   ...
                                                   executeInRemoteThread([this, topBlockIndex]()
                                               ...
                                               [this](const ChainSwitch &msg)
                                               {
                                                   ...
                                                   executeInRemoteThread([this, msg, topBlockIndex]()
                                                                         {
                                                                             chainSwitched(topBlockIndex,
                                                                                           msg.commonRootIndex,
                                                                                           msg.blocksFromCommonRoot);
                                                                             blockchainUpdated(topBlockIndex);
                                                                         });
                                               },
                                               [this](const AddTransaction &msg)
                                               {
                                                   executeInRemoteThread([this]()
                                                                         { poolUpdated(); });
                                               },
...
//may be called from any thread
                                                  ...
                                                  std::vector<CryptoNote::RawBlock> &newBlocks, uint32_t &startHeight)
    {
        ...
        try
        {
            ...
            auto supplement = core.findBlockchainSupplement(knownBlockIds,
            ...
            for (const auto &blockId : supplement)
            {
                ...
                std::move(std::begin(binaryTransactions), std::end(binaryTransactions),
                          std::back_inserter(be.transactions));
    ...
    void InProcessNode::getBlockHashesByTimestamps(uint64_t timestampBegin, size_t secondsCount,
    ...
    void InProcessNode::getTransactionHashesByPaymentId(const Crypto::Hash &paymentId,
    ...
    void
    InProcessNode::chainSwitched(uint32_t topBlockIndex, uint32_t commonRoot, const std::vector<Crypto::Hash> &hashes)
    {
                                    ...
                                    const Callback &callback)
    {
        ...
        executeInDispatcherThread([=, &newBlocks, &startHeight]() mutable
                                  {
                                      auto ec = doQueryBlocksLite(std::move(knownBlockIds), timestamp, newBlocks,
                                                                  startHeight);
                                      executeInRemoteThread([callback, ec]()
                                                            { callback(ec); });
                                  });
    ...
    void
    InProcessNode::getPoolSymmetricDifference(std::vector<Crypto::Hash> &&knownPoolTxIds, Crypto::Hash knownBlockId,
                                              bool &isBcActual,
                                              ...
                                              std::vector<Crypto::Hash> &deletedTxIds, const Callback &callback)
    {
        ...
        executeInDispatcherThread([=, &isBcActual, &newTxs, &deletedTxIds]() mutable
                                  {
                                      auto ec = doGetPoolSymmetricDifference(std::move(knownPoolTxIds), knownBlockId,
                                                                             isBcActual, newTxs, deletedTxIds);
                                      executeInRemoteThread([callback, ec]()
                                                            { callback(ec); });
                                  });
    }
    ...
    std::error_code
    InProcessNode::doGetPoolSymmetricDifference(std::vector<Crypto::Hash> &&knownPoolTxIds, Crypto::Hash knownBlockId,
                                                bool &isBcActual,
                                                std::vector<std::unique_ptr<ITransactionReader>> &newTxs,
    ...
    void
    InProcessNode::getBlocks(const std::vector<uint32_t> &blockHeights, std::vector<std::vector<BlockDetails>> &blocks,
                             const Callback &callback)
    {
        std::unique_lock<std::mutex> lock(mutex);
        if (state != INITIALIZED)
        ...
        executeInDispatcherThread([=, &syncStatus]()
                                  {
                                      ...
                                      executeInRemoteThread([callback]()
                                                            { callback({}); });
                                  });
    }
