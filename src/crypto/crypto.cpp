...
bool crypto_ops::underive_public_key(const KeyDerivation &derivation, size_t output_index, const PublicKey &derived_key,
                                     PublicKey &base)
{
    EllipticCurveScalar scalar;
    ge_p3 point1;
...
std::tuple<bool, std::vector<Signature>>
crypto_ops::generateRingSignatures(const Hash prefixHash, const KeyImage keyImage,
                                   ...
                                   const Crypto::SecretKey transactionSecretKey, uint64_t realOutput)
{
    ...
    for (size_t i = 0; i < publicKeys.size(); i++)
    {
        ...
        else
        {
            ...
            ge_double_scalarmult_base_vartime(&tmp2, reinterpret_cast<unsigned char *>(&signatures[i]), &tmp3,
            ...
            ge_double_scalarmult_precomp_vartime(&tmp2, reinterpret_cast<unsigned char *>(&signatures[i]) + 32, &tmp3,
                                                 reinterpret_cast<unsigned char *>(&signatures[i]), image_pre);
    ...
    sc_sub(reinterpret_cast<unsigned char *>(&signatures[realOutput]), reinterpret_cast<unsigned char *>(&h),
           reinterpret_cast<unsigned char *>(&sum));

    sc_mulsub(reinterpret_cast<unsigned char *>(&signatures[realOutput]) + 32,
              reinterpret_cast<unsigned char *>(&signatures[realOutput]),
              reinterpret_cast<const unsigned char *>(&transactionSecretKey), reinterpret_cast<unsigned char *>(&k));

    return {true, signatures};
}

bool crypto_ops::checkRingSignature(const Hash &prefix_hash, const KeyImage &image, const std::vector<PublicKey> pubs,
                                    const std::vector<Signature> signatures)
{
    ...
    for (size_t i = 0; i < pubs.size(); i++)
    {
        ...
        ge_double_scalarmult_base_vartime(&tmp2, reinterpret_cast<const unsigned char *>(&signatures[i]), &tmp3,
        ...
        ge_double_scalarmult_precomp_vartime(&tmp2, reinterpret_cast<const unsigned char *>(&signatures[i]) + 32, &tmp3,
                                             reinterpret_cast<const unsigned char *>(&signatures[i]), image_pre);
