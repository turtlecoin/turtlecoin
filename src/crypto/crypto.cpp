...
namespace Crypto
{
    ...
    static inline void hash_to_scalar(
        const void *data,
        size_t length,
    ...
    void crypto_ops::generate_deterministic_keys(
        PublicKey &pub,
        SecretKey &sec,
        SecretKey &second
    )
    ...
    SecretKey crypto_ops::generate_m_keys(
        PublicKey &pub,
        SecretKey &sec,
    ...
    bool crypto_ops::secret_key_to_public_key(
        const SecretKey &sec,
        PublicKey &pub
    )
    {
    ...
    bool crypto_ops::generate_key_derivation(
    ...
    )
    {
        ...
        derivation_to_scalar(derivation, output_index, scalar);
        sc_add(
            reinterpret_cast<unsigned char *>(&derived_key), reinterpret_cast<const unsigned char *>(&base), reinterpret_cast<unsigned char *>(&scalar));
    }
    ...
    )
    {
        ...
        sc_mulsub(
            ...
            32, reinterpret_cast<unsigned char *>(&sig), reinterpret_cast<const unsigned char *>(&sec), reinterpret_cast<unsigned char *>(&k));
    }

    bool crypto_ops::check_signature(
        const Hash &prefix_hash,
        const PublicKey &pub,
        const Signature &sig
    )
    {
        ...
        ge_double_scalarmult_base_vartime(
            &tmp2, reinterpret_cast<const unsigned char *>(&sig), &tmp3,
            reinterpret_cast<const unsigned char *>(&sig) + 32
        );
        ge_tobytes(reinterpret_cast<unsigned char *>(&buf.comm), &tmp2);
        hash_to_scalar(&buf, sizeof(s_comm), c);
        sc_sub(
            reinterpret_cast<unsigned char *>(&c), reinterpret_cast<unsigned char *>(&c), reinterpret_cast<const unsigned char *>(&sig));
        return sc_isnonzero(reinterpret_cast<unsigned char *>(&c)) == 0;
    }

    static void hash_to_ec(
        const PublicKey &key,
        ge_p3 &res
    )
    {
    ...
    KeyImage crypto_ops::scalarmultKey(
        const KeyImage &P,
        const KeyImage &a
    )
    {
    ...
    void crypto_ops::hash_data_to_ec(
        const uint8_t *data,
        std::size_t len,
        PublicKey &key
    )
    ...
    void crypto_ops::generate_key_image(
        const PublicKey &pub,
        const SecretKey &sec,
        KeyImage &image
    )
    ...
    std::tuple<
        bool, std::vector<Signature>> crypto_ops::generateRingSignatures(
    ...
    )
    {
        ...
        for (size_t i = 0; i < publicKeys.size(); i++)
        {
            ...
            else
            {
                ...
                random_scalar(
                    *reinterpret_cast<EllipticCurveScalar *>(
                ...
                ge_double_scalarmult_base_vartime(
                    &tmp2, reinterpret_cast<unsigned char *>(&signatures[i]), &tmp3,
                    reinterpret_cast<unsigned char *>(&signatures[i]) + 32
                );
                ...
                ge_double_scalarmult_precomp_vartime(
                    &tmp2, reinterpret_cast<unsigned char *>(&signatures[i]) +
                           32, &tmp3, reinterpret_cast<unsigned char *>(&signatures[i]), image_pre
                );

                ge_tobytes(reinterpret_cast<unsigned char *>(&buf->ab[i].b), &tmp2);

                sc_add(
                    reinterpret_cast<unsigned char *>(&sum), reinterpret_cast<unsigned char *>(&sum), reinterpret_cast<unsigned char *>(&signatures[i]));
            }
        }
        ...
        sc_sub(
            reinterpret_cast<unsigned char *>(&signatures[realOutput]), reinterpret_cast<unsigned char *>(&h), reinterpret_cast<unsigned char *>(&sum));

        sc_mulsub(
            reinterpret_cast<unsigned char *>(&signatures[realOutput]) +
            32, reinterpret_cast<unsigned char *>(&signatures[realOutput]), reinterpret_cast<const unsigned char *>(&transactionSecretKey), reinterpret_cast<unsigned char *>(&k));
    ...
    )
    {
        ...
        for (size_t i = 0; i < pubs.size(); i++)
        {
            ...
            ge_double_scalarmult_base_vartime(
                &tmp2, reinterpret_cast<const unsigned char *>(&signatures[i]), &tmp3,
            ...
            ge_double_scalarmult_precomp_vartime(
                &tmp2, reinterpret_cast<const unsigned char *>(&signatures[i]) +
                       32, &tmp3, reinterpret_cast<const unsigned char *>(&signatures[i]), image_pre
            );

            ge_tobytes(reinterpret_cast<unsigned char *>(&buf->ab[i].b), &tmp2);

            sc_add(
                reinterpret_cast<unsigned char *>(&sum), reinterpret_cast<unsigned char *>(&sum), reinterpret_cast<const unsigned char *>(&signatures[i]));
        }

        hash_to_scalar(buf, rs_comm_size(pubs.size()), h);

        sc_sub(
            reinterpret_cast<unsigned char *>(&h), reinterpret_cast<unsigned char *>(&h), reinterpret_cast<unsigned char *>(&sum));
