...
//--------------------------------------------------------------------------------
bool DaemonCommandsHandler::print_block_raw(const std::vector<std::string> &args) {
  if (args.empty()) {
    std::cout << "expected: print_block_raw <block_height>" << std::endl;
    return true;
  }
  ...
  try {
    uint32_t height = boost::lexical_cast<uint32_t>(arg);
    print_block_raw_by_height(height);
  } catch (boost::bad_lexical_cast &) {
...
//--------------------------------------------------------------------------------
bool DaemonCommandsHandler::print_block_raw_by_height(uint32_t height)
{
  if (height - 1 > m_core.getTopBlockIndex()) {
    std::cout << "block wasn't found. Current block chain height: " << m_core.getTopBlockIndex() + 1 << ", requested: " << height << std::endl;
    return false;
  ...
  std::cout << ENDL << m_core.getRawBlockHex(height - 1) << ENDL;
