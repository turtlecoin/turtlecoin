...
namespace Miner
{
    ...
    void MinerManager::start()
    {
        ...
        for (;;)
        {
            ...
            try
            {
                params = requestMiningParameters(m_dispatcher, m_config.daemonHost, m_config.daemonPort,
            ...
            switch (event.type)
            {
                case MinerEventType::BLOCK_MINED:
                {
                    ...
                    if (submitBlock(m_minedBlock, m_config.daemonHost, m_config.daemonPort))
                    {
                        ...
                        {
                            m_logger(Logging::INFO) << "Miner mined requested " << m_config.blocksLimit
                                                    << " blocks. Quitting";
                            return;
                        }
                    }

                    BlockMiningParameters params = requestMiningParameters(m_dispatcher, m_config.daemonHost,
                                                                           m_config.daemonPort, m_config.miningAddress);
                    adjustBlockTemplate(params.blockTemplate);
        ...
        m_contextGroup.spawn([this]()
                             {
                                 ...
                                 {
                                     m_logger(Logging::ERROR) << "BlockchainMonitor context unexpectedly finished: "
                                                              << e.what();
                                 }
                             });
    }
    ...
    bool
    MinerManager::submitBlock(const BlockTemplate &minedBlock, const std::string &daemonHost, uint16_t daemonPort)
    {
    ...
    BlockMiningParameters
    MinerManager::requestMiningParameters(System::Dispatcher &dispatcher, const std::string &daemonHost,
                                          uint16_t daemonPort, const std::string &miningAddress)
    {
        try
        {
