...
WalletGreen::WalletGreen(System::Dispatcher &dispatcher, const Currency &currency, INode &node,
    ...
    : m_dispatcher(dispatcher), m_currency(currency), m_node(node), m_logger(logger, "WalletGreen/empty"),
      m_stopped(false), m_blockchainSynchronizerStarted(false),
      m_blockchainSynchronizer(node, logger, currency.genesisBlockHash()),
      m_synchronizer(currency, logger, m_blockchainSynchronizer, node), m_eventOccurred(m_dispatcher),
      m_readyEvent(m_dispatcher), m_state(WalletState::NOT_INITIALIZED), m_actualBalance(0), m_pendingBalance(0),
...
void WalletGreen::createViewWallet(const std::string &path, const std::string &password, const std::string address,
                                   const Crypto::SecretKey &viewSecretKey, const uint64_t scanHeight, bool newAddress)
{
                                  ...
                                  std::unordered_set<Crypto::PublicKey> &deletedKeys, std::string &extra)
{
    ...
    WalletSerializerV2 s(*this, m_viewPublicKey, m_viewSecretKey, m_actualBalance, m_pendingBalance, m_walletsContainer,
                         m_synchronizer, m_unlockTransactionsJob, m_transactions, m_transfers, m_uncommitedTransactions,
                         extra, m_transactionSoftLockTime);
                                  ...
                                  const std::string &extra)
{
    ...
    WalletSerializerV2 s(*this, m_viewPublicKey, m_viewSecretKey, m_actualBalance, m_pendingBalance, m_walletsContainer,
                         m_synchronizer, m_unlockTransactionsJob, transactions, transfers, m_uncommitedTransactions,
...
void WalletGreen::subscribeWallets()
{
    ...
    try
    {
        ...
        for (auto it = index.begin(); it != index.end(); ++it)
        {
            ...
            bool r =
                index.modify(it, [&subscription](WalletRecord &rec) { rec.container = &subscription.getContainer(); });
            if (r)
            {
            }
            assert(r);
    ...
    for (size_t i = 0; i < spendSecretKeys.size(); ++i)
    {
        ...
        if (!Crypto::secret_key_to_public_key(spendSecretKeys[i], spendPublicKey))
        {
            m_logger(ERROR, BRIGHT_RED)
                << "createAddressList(): failed to convert secret key to public key, secret key " << spendSecretKeys[i];
            throw std::system_error(make_error_code(CryptoNote::error::KEY_GENERATION_ERROR));
        }
...
std::string WalletGreen::doCreateAddress(const Crypto::PublicKey &spendPublicKey,
                                         const Crypto::SecretKey &spendSecretKey, const uint64_t scanHeight,
                                         const bool newAddress)
{
...
std::string WalletGreen::addWallet(const NewAddressData &addressData, uint64_t scanHeight, bool newAddress)
{
    ...
    {
        m_logger(ERROR, BRIGHT_RED)
            << "Failed to add wallet: incompatible tracking mode and spend secret key, tracking mode=" << trackingMode
...
size_t WalletGreen::transfer(const PreparedTransaction &preparedTransaction)
{
    ...
    id =
        validateSaveAndSendTransaction(*preparedTransaction.transaction, preparedTransaction.destinations, false, true);
    return id;
}
...
size_t WalletGreen::transfer(const TransactionParameters &transactionParameters)
{
    ...
    Tools::ScopeExit releaseContext([this, &id] {
        ...
        if (id != WALLET_INVALID_TRANSACTION_ID)
        {
            ...
            m_logger(INFO, BRIGHT_WHITE) << "Transaction created and send, ID " << id << ", hash "
                                         << m_transactions[id].hash << ", state " << tx.state << ", totalAmount "
                                         << m_currency.formatAmount(tx.totalAmount) << ", fee "
                                         << m_currency.formatAmount(tx.fee) << ", transfers: "
                                         << TransferListFormatter(m_currency, getTransactionTransfersRange(id));
        }
    ...
    m_logger(INFO, BRIGHT_WHITE) << "transfer"
                                 ...
                                 << ", change address '" << transactionParameters.changeDestination << '\'' << ", fee "
                                 << m_currency.formatAmount(transactionParameters.fee) << ", mixin "
...
uint64_t WalletGreen::getBalanceMinusDust(const std::vector<std::string> &addresses)
{
    ...
    return selectTransfers(needed,
                           ...
                           false, m_currency.defaultDustThreshold(m_node.getLastKnownBlockHeight()), std::move(wallets),
                           unused);
}
                                     ...
                                     PreparedTransaction &preparedTransaction)
{
    ...
    uint64_t donationAmount = pushDonationTransferIfPossible(
        ...
        m_currency.defaultDustThreshold(m_node.getLastKnownBlockHeight()), preparedTransaction.destinations);
    ...
    std::vector<ReceiverAmounts> decomposedOutputs =
        splitDestinations(preparedTransaction.destinations,
                          m_currency.defaultDustThreshold(m_node.getLastKnownBlockHeight()), m_currency);
...
void WalletGreen::checkIfEnoughMixins(std::vector<RandomOuts> &mixinResult, uint16_t mixIn) const
{
    ...
    auto notEnoughIt = std::find_if(mixinResult.begin(), mixinResult.end(),
                                    [mixIn](const auto ofa) { return ofa.outs.size() < mixIn; });
...
WalletGreen::makeTransaction(const std::vector<ReceiverAmounts> &decomposedOutputs, std::vector<InputInfo> &keysInfo,
                                                   ...
                                                   bool send)
{
    ...
    m_logger(DEBUGGING) << "Transaction added to container, ID " << transactionId << ", hash "
                        << transaction.getTransactionHash() << ", block " << m_transactions[transactionId].blockHeight
...
void WalletGreen::requestMixinOuts(const std::vector<OutputToTransfer> &selectedTransfers, uint16_t mixIn,
                                   std::vector<CryptoNote::RandomOuts> &mixinResult)
{
    ...
    System::RemoteContext<void> getOutputsContext(
        m_dispatcher, [this, amounts, requestMixinCount, &mixinResult, &requestFinished, &mixinError]() mutable {
            m_node.getRandomOutsByAmounts(std::move(amounts), requestMixinCount, mixinResult,
                                          [&requestFinished, &mixinError, this](std::error_code ec) mutable {
                                              mixinError = ec;
                                              m_dispatcher.remoteSpawn(
                                                  std::bind(asyncRequestCompletion, std::ref(requestFinished)));
                                          });
        });
...
WalletGreen::splitDestinations(const std::vector<CryptoNote::WalletTransfer> &destinations, uint64_t dustThreshold,
...
void WalletGreen::prepareInputs(const std::vector<OutputToTransfer> &selectedTransfers,
                                std::vector<CryptoNote::RandomOuts> &mixinResult, uint16_t mixIn,
                                std::vector<InputInfo> &keysInfo)
{
    ...
    for (const auto &input : selectedTransfers)
    {
        ...
        auto insertIn =
            std::find_if(keyInfo.outputs.begin(), keyInfo.outputs.end(), [&](const TransactionTypes::GlobalOutput &a) {
...
{
    // Deprecated, ignore it. New event handler is onTransactionUpdated(const Crypto::PublicKey&, const Crypto::Hash&,
    // const std::vector<ITransfersContainer*>&)
}
                                            ...
                                            const std::string &destinationAddress)
{
    ...
    m_logger(INFO, BRIGHT_WHITE) << "createFusionTransaction"
                                 ...
                                 << destinationAddress << '\'' << ", threshold " << m_currency.formatAmount(threshold)
                                 << ", mixin " << mixin;
...
WalletGreen::pickRandomFusionInputs(const std::vector<std::string> &addresses, uint64_t threshold, size_t minInputCount,
...
CryptoNote::AccountPublicAddress
WalletGreen::getChangeDestination(const std::string &changeDestinationAddress,
                                  const std::vector<std::string> &sourceAddresses) const
{
