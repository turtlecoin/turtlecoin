...
namespace CryptoNote
{
    ...
    WalletGreen::encryptKeyPair(const PublicKey &publicKey, const SecretKey &secretKey, uint64_t creationTimestamp,
                                   ...
                                   const Crypto::PublicKey &viewPublicKey, const Crypto::SecretKey &viewSecretKey)
    {
        ...
        prefix->encryptedViewKeys = encryptKeyPair(viewPublicKey, viewSecretKey, creationTimestamp, m_key,
                                                   prefix->nextIv);
    ...
    void WalletGreen::loadContainerStorage(const std::string &path)
    {
        try
        {
            ...
            m_logger = Logging::LoggerRef(m_logger.getLogger(),
                                          "WalletGreen/" + podToHex(m_viewPublicKey).substr(0, 5));
    ...
    void
    WalletGreen::saveWalletCache(ContainerStorage &storage, const Crypto::chacha8_key &key, WalletSaveLevel saveLevel,
                                 const std::string &extra)
    {
    ...
    void
    WalletGreen::copyContainerStoragePrefix(ContainerStorage &src, const chacha8_key &srcKey, ContainerStorage &dst,
                                            const chacha8_key &dstKey)
    {
        ...
        dstPrefix->encryptedViewKeys = encryptKeyPair(publicKey, secretKey, creationTimestamp, dstKey,
                                                      dstPrefix->nextIv);
        incIv(dstPrefix->nextIv);
    }
    ...
    void WalletGreen::deleteOrphanTransactions(const std::unordered_set<Crypto::PublicKey> &deletedKeys)
    {
        for (auto spendPublicKey : deletedKeys)
        {
            ...
            std::vector<size_t> updatedTransactions = deleteTransfersForAddress(deletedAddressString,
                                                                                deletedTransactions);
        ...
        try
        {
            ...
            for (auto it = index.begin(); it != index.end(); ++it)
            {
                ...
                sub.syncStart.timestamp =
                        std::max(static_cast<uint64_t>(wallet.creationTimestamp), ACCOUNT_CREATE_TIME_ACCURACY) -
                        ACCOUNT_CREATE_TIME_ACCURACY;

                auto &subscription = m_synchronizer.addSubscription(sub);
                bool r = index.modify(it, [&subscription](
                        WalletRecord &rec)
    ...
    void WalletGreen::convertAndLoadWalletFile(const std::string &path, std::ifstream &&walletFileStream)
    {
        ...
        m_containerStorage.open(tmpPath.string(), Common::FileMappedVectorOpenMode::CREATE,
        ...
        m_containerStorage.atomicUpdate([this, newKey](ContainerStorage &newStorage)
                                        {
                                            ...
                                            if (m_containerStorage.suffixSize() > 0)
                                            {
                                                ...
                                                encryptAndSaveContainerData(newStorage, newKey, containerData.data(),
                                                                            containerData.size());
                                            }
                                        });
    ...
    std::string
    WalletGreen::doCreateAddress(const Crypto::PublicKey &spendPublicKey, const Crypto::SecretKey &spendSecretKey,
                                 uint64_t creationTimestamp)
    {
    ...
    std::vector<std::string> WalletGreen::doCreateAddressList(const std::vector<NewAddressData> &addressDataList)
    {
        ...
        try
        {
...

            {
                ...
                {
                    assert(addressData.creationTimestamp <=
                           std::numeric_limits<uint64_t>::max() - m_currency.blockFutureTimeLimit());
                    std::string address = addWallet(addressData.spendPublicKey, addressData.spendSecretKey,
                                                    addressData.creationTimestamp);
                    m_logger(INFO, BRIGHT_WHITE) << "New wallet added " << address << ", creation timestamp "
                                                 << addressData.creationTimestamp;
    ...
    std::string WalletGreen::addWallet(const Crypto::PublicKey &spendPublicKey, const Crypto::SecretKey &spendSecretKey,
                                       uint64_t creationTimestamp)
    {
        ...
        try
        {
            ...
            sub.syncStart.height = 0;
            sub.syncStart.timestamp =
                    std::max(creationTimestamp, ACCOUNT_CREATE_TIME_ACCURACY) - ACCOUNT_CREATE_TIME_ACCURACY;
    ...
    void WalletGreen::deleteAddress(const std::string &address)
    {
        ...
        if (it->actualBalance != 0 || it->pendingBalance != 0)
        {
            m_logger(INFO, BRIGHT_WHITE) << "Container balance updated, actual "
                                         << m_currency.formatAmount(m_actualBalance) <<
    ...
    WalletGreen::TransfersRange WalletGreen::getTransactionTransfersRange(size_t transactionIndex) const
    {
        ...
        auto bounds = std::equal_range(m_transfers.begin(), m_transfers.end(), val,
                                       [](const TransactionTransferPair &a, const TransactionTransferPair &b)
                                       {
        ...
        Tools::ScopeExit releaseContext([this, &id]
                                        {
                                            ...
                                            if (id != WALLET_INVALID_TRANSACTION_ID)
                                            {
                                                ...
                                                m_logger(INFO, BRIGHT_WHITE) << "Transaction created and send, ID "
                                                                             << id <<
                                                                             ", hash " << m_transactions[id].hash <<
                                                                             ", state " << tx.state <<
                                                                             ", totalAmount "
                                                                             << m_currency.formatAmount(tx.totalAmount)
                                                                             <<
                                                                             ", fee " << m_currency.formatAmount(tx.fee)
                                                                             <<
                                                                             ", transfers: "
                                                                             << TransferListFormatter(m_currency,
                                                                                                      getTransactionTransfersRange(
                                                                                                              id));
                                            }
                                        });
                                         ...
                                         PreparedTransaction &preparedTransaction)
    {
        ...
        std::vector<ReceiverAmounts> decomposedOutputs = splitDestinations(preparedTransaction.destinations,
                                                                           m_currency.defaultDustThreshold(
                                                                                   m_node.getLastKnownBlockHeight()),
                                                                           m_currency);
                                                         ...
                                                         std::vector<WalletTransfer> &destinations) const
    {
        ...
        if (!donation.address.empty() && donation.threshold != 0)
        {
            ...
            if (donationAmount != 0)
            {
                destinations.emplace_back(WalletTransfer{WalletTransferType::DONATION, donation.address,
        ...
        if (changeDestination.empty())
        {
            if (sourceAddresses.size() > 1 || (sourceAddresses.empty() && m_walletsContainer.size() > 1))
            {
                ...
                throw std::system_error(make_error_code(
                        isFusion ? error::DESTINATION_ADDRESS_REQUIRED : error::CHANGE_ADDRESS_REQUIRED), message);
            }
        } else
        {
            if (!CryptoNote::validateAddress(changeDestination, m_currency))
            {
                message = std::string("Bad ") + (isFusion ? "destination" : "change destination") + " address: " +
                          changeDestination;
                m_logger(ERROR, BRIGHT_RED) << message;
            ...
            if (!isMyAddress(changeDestination))
            {
                ...
                m_logger(ERROR, BRIGHT_RED) << message;
                throw std::system_error(make_error_code(
                        isFusion ? error::DESTINATION_ADDRESS_NOT_FOUND : error::CHANGE_ADDRESS_NOT_FOUND), message);
            }
        }
    }
    ...
    void WalletGreen::validateTransactionParameters(const TransactionParameters &transactionParameters) const
    {
        ...
        if (transactionParameters.donation.address.empty() != (transactionParameters.donation.threshold == 0))
        {
            std::string message = "DonationSettings must have both address and threshold parameters filled. Address '" +
                                  transactionParameters.donation.address + "'" +
    ...
    size_t WalletGreen::doTransfer(const TransactionParameters &transactionParameters)
    {
        ...
        CryptoNote::AccountPublicAddress changeDestination = getChangeDestination(
                transactionParameters.changeDestination, transactionParameters.sourceAddresses);
    ...
    size_t WalletGreen::getTxSize(const TransactionParameters &sendingTransaction)
    {
        ...
        CryptoNote::AccountPublicAddress changeDestination = getChangeDestination(sendingTransaction.changeDestination,
    ...
    void WalletGreen::commitTransaction(size_t transactionId)
    {
        ...
        {
            m_node.relayTransaction(m_uncommitedTransactions[transactionId],
                                    [&ec, &completion, this](std::error_code error)
                                    {
                                        ec = error;
                                        this->m_dispatcher.remoteSpawn(
                                                std::bind(asyncRequestCompletion, std::ref(completion)));
                                    });
        });
    ...
    bool WalletGreen::updateUnknownTransfers(size_t transactionId, size_t firstTransferIdx,
                                             ...
                                             int64_t knownAmount, int64_t myAmount, int64_t totalAmount,
                                             bool isOutput)
    {
                                     ...
                                     int64_t amount)
    {
        ...
        auto insertIt = std::upper_bound(it, m_transfers.end(), transactionId,
                                         [](size_t transactionId, const TransactionTransferPair &pair)
                                         {
                                            ...
                                            bool eraseOutputTransfers)
    {
        ...
        return eraseTransfers(transactionId, firstTransferIdx,
                              [this, &knownAddresses, eraseOutputTransfers](bool isOutput,
    ...
    WalletGreen::makeTransaction(const std::vector<ReceiverAmounts> &decomposedOutputs,
                                 std::vector<InputInfo> &keysInfo, const std::string &extra, uint64_t unlockTimestamp)
    {
        ...
        std::shuffle(amountsToAddresses.begin(), amountsToAddresses.end(),
    ...
    void WalletGreen::sendTransaction(const CryptoNote::Transaction &cryptoNoteTransaction)
    {
        ...
        System::RemoteContext<void> relayTransactionContext(m_dispatcher,
                                                            [this, &cryptoNoteTransaction, &ec, &completion]()
                                                            {
                                                                m_node.relayTransaction(cryptoNoteTransaction,
                                                                                        [&ec, &completion, this](
                                                       ...
                                                       bool send)
    {
        ...
        size_t transactionId = insertOutgoingTransactionAndPushEvent(transaction.getTransactionHash(), fee,
                                                                     transaction.getExtra(),
                                                                     transaction.getUnlockTime());
        ...
        Tools::ScopeExit rollbackTransactionInsertion([this, transactionId]
                                                      {
                                                          updateTransactionStateAndPushEvent(transactionId,
        ...
        Tools::ScopeExit rollbackAddingUnconfirmedTransaction([this, &transaction]
                                                              {
                                                                  ...
                                                                  } catch (...)
                                                                  {
                                                                      ...
                                                                      m_logger(ERROR, BRIGHT_RED)
                                                                              << "Unknown exception while removing unconfirmed transaction "
                                                                              << transaction.getTransactionHash();
                                                                  }
                                                              });
            ...
            std::vector<CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> &mixinResult)
    {
        ...
        System::RemoteContext<void> getOutputsContext(m_dispatcher,
                                                      [this, amounts, requestMixinCount, &mixinResult, &requestFinished, &mixinError]() mutable
                                                      {
                                                          m_node.getRandomOutsByAmounts(std::move(amounts),
                                                                                        requestMixinCount, mixinResult,
                                                                                        [&requestFinished, &mixinError, this](
    ...
    std::vector<CryptoNote::WalletGreen::ReceiverAmounts>
    WalletGreen::splitDestinations(const std::vector<CryptoNote::WalletTransfer> &destinations,
                                   uint64_t dustThreshold,
            ...
            std::vector<InputInfo> &keysInfo)
    {
        ...
        for (const auto &input: selectedTransfers)
        {
            ...
            auto insertIn = std::find_if(keyInfo.outputs.begin(), keyInfo.outputs.end(),
    ...
    std::vector<TransactionsInBlockInfo>
    WalletGreen::getTransactions(const Crypto::Hash &blockHash, size_t count) const
    {
        throwIfNotInitialized();
        throwIfStopped();
        ...
        uint32_t blockIndex = static_cast<uint32_t>(std::distance(m_blockchain.get<BlockHeightIndex>().begin(),
                                                                  heightIt));
...
// TODO remove
                                         ...
                                         const std::vector<ContainerAmounts> &containerAmountsList)
    {
        ...
        m_logger(DEBUGGING) << "transactionUpdated event, hash " << transactionInfo.transactionHash <<
                            ...
                            (transactionInfo.paymentId == NULL_HASH ? "" : ", paymentId " +
        ...
        int64_t totalAmount = std::accumulate(containerAmountsList.begin(), containerAmountsList.end(),
                                              static_cast<int64_t>(0),
                                              [](int64_t sum, const ContainerAmounts &containerAmounts)
                                              {
                                                  return sum + containerAmounts.amounts.input +
                                                         containerAmounts.amounts.output;
                                              });
        ...
        if (it != hashIndex.end())
        {
            transactionId = std::distance(m_transactions.get<RandomAccessIndex>().begin(),
                                          m_transactions.project<RandomAccessIndex>(it));
        ...
        for (auto containerAmounts : containerAmountsList)
        {
            ...
            if (transactionInfo.blockHeight != CryptoNote::WALLET_UNCONFIRMED_TRANSACTION_HEIGHT)
            {
                uint32_t unlockHeight = std::max(transactionInfo.blockHeight + m_transactionSoftLockTime,
        ...
        if (isNew)
        {
            ...
            m_logger(INFO, BRIGHT_WHITE) << "New transaction received, ID " << transactionId <<
                                         ...
                                         ", fee " << m_currency.formatAmount(tx.fee) <<
                                         ", transfers: " << TransferListFormatter(m_currency,
                                                                                  getTransactionTransfersRange(
                                                                                          transactionId));
    ...
    void WalletGreen::transactionDeleted(ITransfersSubscription *object, const Hash &transactionHash)
    {
        ...
        bool updated = false;
        m_transactions.get<TransactionIndex>().modify(it, [this, &transactionHash, &updated](
    ...
    void WalletGreen::insertUnlockTransactionJob(const Hash &transactionHash, uint32_t blockHeight,
...
/* The blockchain events are sent to us from the blockchain synchronizer,
   ...
   archaic line of code to run other code on the dispatcher? */
                                                ...
                                                const std::string &destinationAddress)
    {
        ...
        Tools::ScopeExit releaseContext([this, &id]
                                        {
                                            ...
                                            if (id != WALLET_INVALID_TRANSACTION_ID)
                                            {
                                                ...
                                                m_logger(INFO, BRIGHT_WHITE)
                                                        ...
                                                        ", transfers: "
                                                        << TransferListFormatter(m_currency,
                                                                                 getTransactionTransfersRange(id));
                                            }
                                        });
        ...
        if (threshold <= m_currency.defaultDustThreshold(height))
        {
            m_logger(ERROR, BRIGHT_RED) << "Fusion transaction threshold is too small. Threshold "
                                        ...
                                        ", minimum threshold "
                                        << m_currency.formatAmount(m_currency.defaultDustThreshold(height) + 1);
            throw std::runtime_error("Threshold must be greater than " +
                                     m_currency.formatAmount(m_currency.defaultDustThreshold(height)));
        }
        ...
        size_t estimatedFusionInputsCount = m_currency.getApproximateMaximumInputCount(m_currency.fusionTxMaxSize(),
                                                                                       MAX_FUSION_OUTPUT_COUNT, mixin);
        ...
        do
        {
            ...
            uint64_t inputsAmount = std::accumulate(fusionInputs.begin(), fusionInputs.end(), static_cast<uint64_t>(0),
                                                    [](uint64_t amount, const OutputToTransfer &input)
                                                    {
    ...
    bool WalletGreen::isFusionTransaction(const WalletTransaction &walletTx) const
    {
        ...
        {
            for (const TransactionOutputInformation &output : wallet.container->getTransactionOutputs(walletTx.hash,
                                                                                                      ITransfersContainer::IncludeTypeKey |
                                                                                                      ITransfersContainer::IncludeStateAll))
            {
            ...
            for (const TransactionOutputInformation &input : wallet.container->getTransactionInputs(walletTx.hash,
                                                                                                    ITransfersContainer::IncludeTypeKey))
            {
    ...
    std::vector<WalletGreen::OutputToTransfer>
    WalletGreen::pickRandomFusionInputs(const std::vector<std::string> &addresses,
...
///pre: source address belongs to current container
    CryptoNote::AccountPublicAddress WalletGreen::getChangeDestination(const std::string &changeDestinationAddress,
    ...
    WalletGreen::deleteTransfersForAddress(const std::string &address, std::vector<size_t> &deletedTransactions)
    {
        ...
        for (size_t i = 0; i < m_transfers.size(); ++i)
        {
            ...
            if ((i == m_transfers.size() - 1) || (transactionId != m_transfers[i + 1].first))
            {
                ...
                randomIndex.modify(std::next(randomIndex.begin(), transactionId),
