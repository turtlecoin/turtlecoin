...
BlockchainSynchronizer::BlockchainSynchronizer(INode &node, std::shared_ptr<Logging::ILogger> logger,
                                               const Hash &genesisBlockHash)
    : m_logger(logger, "BlockchainSynchronizer"), m_node(node), m_genesisBlockHash(genesisBlockHash),
      m_currentState(State::stopped), m_futureState(State::stopped)
{
}
...
void BlockchainSynchronizer::startBlockchainSync()
{
    ...
    try
    {
        if (!req.knownBlocks.empty())
        {
            ...
            m_node.queryBlocks(std::move(req.knownBlocks), req.syncStart.timestamp, response.newBlocks,
                               response.startHeight, [&queryBlocksCompleted](std::error_code ec) {
...
void BlockchainSynchronizer::processBlocks(GetBlocksResponse &response)
{
    ...
    if (!checkIfShouldStop())
    {
        ...
        case UpdateConsumersResult::addedNewBlocks:
            setFutureState(State::blockchainSync);
            m_observerManager.notify(&IBlockchainSynchronizerObserver::synchronizationProgressUpdated,
                                     processedBlockCount,
...
BlockchainSynchronizer::UpdateConsumersResult
BlockchainSynchronizer::updateConsumers(const BlockchainInterval &interval, const std::vector<CompleteBlock> &blocks)
{
...
std::error_code BlockchainSynchronizer::processPoolTxs(GetPoolResponse &response)
{
    m_logger(DEBUGGING) << "Starting to process pool transactions, added " << response.newTxs.size() << ':'
                        ...
                        << response.deletedTxIds.size() << ':' << Common::makeContainerFormatter(response.deletedTxIds);
