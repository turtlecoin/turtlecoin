...
namespace CryptoNote
{
                                                    ...
                                                    const std::vector<ITransfersContainer *> &containers)
    {
        ...
        if (it != m_subscribers.end())
        {
            it->second->notify(&ITransfersSynchronizerObserver::onTransactionUpdated, it->first, transactionHash,
                               containers);
        }
    }
                                                              ...
                                                              ITransfersSynchronizerObserver *observer)
    {
        ...
        auto insertedIt = m_subscribers.emplace(viewPublicKey,
                                                std::unique_ptr<SubscribersNotifier>(new SubscribersNotifier())).first;
