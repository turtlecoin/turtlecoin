...
namespace CryptoNote
{
    ...
    TransfersContainer::TransfersContainer(const Currency &currency, Logging::ILogger &logger,
                                            ...
                                            const std::vector<TransactionOutputInformationIn> &transfers)
    {
        ...
        try
        {
            ...
            if (block.height < m_currentHeight)
            {
                ...
                m_logger(ERROR, BRIGHT_RED) << message << ", block " << block.height << ", m_currentHeight "
                                            << m_currentHeight;
        ...
        for (const auto &transfer : transfers)
        {
            ...
            } else
            {
                if (info.type == TransactionTypes::OutputType::Key)
                {
                    ...
                    auto availableRange = m_availableTransfers.get<SpentOutputDescriptorIndex>().equal_range(
                            descriptor);
    ...
    bool TransfersContainer::addTransactionInputs(const TransactionBlockInfo &block, const ITransactionReader &tx)
    {
        ...
        for (size_t i = 0; i < tx.getInputCount(); ++i)
        {
            ...
            if (inputType == TransactionTypes::InputType::Key)
            {
                ...
                auto unconfirmedRange = m_unconfirmedTransfers.get<SpentOutputDescriptorIndex>().equal_range(
                        descriptor);
 ...
 */
    void TransfersContainer::deleteTransactionTransfers(const Hash &transactionHash)
    {
        ...
        auto unconfirmedTransfersRange = m_unconfirmedTransfers.get<ContainingTransactionIndex>().equal_range(
                transactionHash);
 ...
 */
                                                       ...
                                                       uint64_t *amountIn, uint64_t *amountOut) const
    {
        ...
        if (amountOut != nullptr)
        {
            *amountOut = 0;
            ...
            {
                auto unconfirmedOutputsRange = m_unconfirmedTransfers.get<ContainingTransactionIndex>().equal_range(
                        transactionHash);
            ...
            {
                auto availableOutputsRange = m_availableTransfers.get<ContainingTransactionIndex>().equal_range(
                        transactionHash);
                ...
                auto spentOutputsRange = m_spentTransfers.get<ContainingTransactionIndex>().equal_range(
                        transactionHash);
                                                                                        ...
                                                                                        uint32_t flags) const
    {
        ...
        if ((flags & IncludeStateLocked) != 0)
        {
            auto unconfirmedRange = m_unconfirmedTransfers.get<ContainingTransactionIndex>().equal_range(
                    transactionHash);
    ...
    void TransfersContainer::save(std::ostream &os)
    {
        ...
        writeSequence<TransactionOutputInformationEx>(m_unconfirmedTransfers.begin(), m_unconfirmedTransfers.end(),
                                                      "unconfirmedTransfers", s);
        writeSequence<TransactionOutputInformationEx>(m_availableTransfers.begin(), m_availableTransfers.end(),
                                                      "availableTransfers", s);
    ...
    void TransfersContainer::load(std::istream &in)
    {
        ...
        readSequence<TransactionOutputInformationEx>(std::inserter(unconfirmedTransfers, unconfirmedTransfers.end()),
                                                     "unconfirmedTransfers", s);
        readSequence<TransactionOutputInformationEx>(std::inserter(availableTransfers, availableTransfers.end()),
                                                     "availableTransfers", s);
        ...
        for (auto it = m_spentTransfers.begin(); it != m_spentTransfers.end();)
        {
            ...
            if (m_transactions.count(it->spendingTransactionHash) == 0)
            {
                ...
                m_logger(WARNING, BRIGHT_YELLOW)
                        ...
                        "    input       " <<
                        ": block " << std::setw(7)
                        << (isInputConfirmed ? static_cast<int32_t>(it->spendingBlock.height) : -1) <<
                        ", transaction index " << std::setw(2)
                        << (isInputConfirmed ? static_cast<int32_t>(it->spendingBlock.transactionIndex) : -1) <<
