...
template <typename TIterator> class TransferIteratorList
{
  public:
    ...
    {
        auto listIt =
            std::find_if(m_list.begin(), m_list.end(), [amount](const TIterator &it) { return it->amount == amount; });
...
SpentOutputDescriptor::SpentOutputDescriptor(const TransactionOutputInformationIn &transactionInfo)
    : m_type(transactionInfo.type), m_amount(0), m_globalOutputIndex(0)
{
...
TransfersContainer::TransfersContainer(const Currency &currency, std::shared_ptr<Logging::ILogger> logger,
    ...
    : m_currentHeight(0), m_currency(currency), m_logger(logger, "TransfersContainer"),
        ...
        if (inputType == TransactionTypes::InputType::Key)
        {
            ...
            if (std::distance(spentRange.first, spentRange.second) > 0)
            {
                ...
                m_logger(ERROR, BRIGHT_RED)
                    ...
                    << spentOutput.outputInTransaction << ", amount " << m_currency.formatAmount(spentOutput.amount);
                throw std::runtime_error(message);
            }
...
std::vector<TransactionOutputInformation> TransfersContainer::getTransactionInputs(const Hash &transactionHash,
                                                                                   uint32_t flags) const
{
