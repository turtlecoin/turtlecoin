...
bool PeerlistManager::get_peerlist_head(std::list<PeerlistEntry> &bs_head, uint32_t depth)
{
    ...
    std::sort(m_peers_white.begin(), m_peers_white.end(),
...
bool PeerlistManager::append_with_peer_white(const PeerlistEntry &newPeer)
{
    try
    {
        ...
        auto whiteListIterator = std::find_if(m_peers_white.begin(), m_peers_white.end(),
        ...
        auto grayListIterator = std::find_if(m_peers_gray.begin(), m_peers_gray.end(),
                                             [&newPeer](const auto peer) { return peer.adr == newPeer.adr; });
...
Peerlist &PeerlistManager::getWhite() { return m_whitePeerlist; }

Peerlist &PeerlistManager::getGray() { return m_grayPeerlist; }
