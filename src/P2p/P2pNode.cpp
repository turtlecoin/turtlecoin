...
P2pNode::P2pNode(const P2pNodeConfig &cfg, Dispatcher &dispatcher, std::shared_ptr<Logging::ILogger> log,
                 const Crypto::Hash &genesisHash, uint64_t peerId)
    : logger(log, "P2pNode:" + std::to_string(cfg.getBindPort())), m_stopRequested(false), m_cfg(cfg),
      m_myPeerId(peerId), m_genesisPayload(CORE_SYNC_DATA{1, genesisHash}), m_dispatcher(dispatcher),
      workingContextGroup(dispatcher), m_connectorTimer(dispatcher), m_queueEvent(dispatcher)
{
...
uint64_t P2pNode::getPeerId() const { return m_myPeerId; }
