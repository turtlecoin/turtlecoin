...
BlockchainExplorer::BlockchainExplorer(INode &node, std::shared_ptr<Logging::ILogger> logger)
    : node(node), logger(logger, "BlockchainExplorer"), state(NOT_INITIALIZED), synchronized(false), observersCounter(0)
{
}
...
void BlockchainExplorer::poolChanged()
{
    ...
    request.performAsync(asyncContextCounter, [this, rawNewTransactionsPtr, removedTransactionsPtr,
        ...
        request.performAsync(asyncContextCounter, [this, newTransactionsHashesPtr, newTransactionsPtr,
            ...
            {
                observerManager.notify(&IBlockchainObserver::poolUpdated, *newTransactionsPtr,
