...
namespace CryptoNote
{
                                       ...
                                       std::vector<BlockDetails> &blocks, uint32_t &blocksNumberWithinTimestamps)
    {
        ...
        NodeRequest request([&](const INode::Callback &cb)
                            {
                                node.getBlockHashesByTimestamps(timestampBegin, timestampEnd - timestampBegin + 1,
                                                                blockHashes, cb);
                            });
    ...
    void BlockchainExplorer::poolChanged()
    {
        ...
        request.performAsync(asyncContextCounter,
                                     ...
                                     std::error_code ec)
                             {
                                 ...
                                 NodeRequest request([&](const INode::Callback &cb)
                                                     {
                                                         node.getTransactions(*newTransactionsHashesPtr,
                                                                              *newTransactionsPtr, cb);
                                                     });
    ...
    void BlockchainExplorer::blockchainSynchronized(uint32_t topIndex)
    {
        ...
        request.performAsync(asyncContextCounter,
                             [this, blockIndexesPtr, blocksPtr, topIndex](std::error_code ec)
                             {
                                 ...
                                 auto mainchainBlockIter = std::find_if_not(blocksPtr->front().cbegin(),
                                                                            blocksPtr->front().cend(),
                                                                            [](const BlockDetails &block)
                                                                            { return block.isAlternative; });
                                 assert(mainchainBlockIter != blocksPtr->front().cend());

                                 observerManager.notify(&IBlockchainObserver::blockchainSynchronized,
                                                        *mainchainBlockIter);
