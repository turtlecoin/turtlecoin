...
)
{
    ...
    rawDataToInsert.emplace_back(
        DB::serialize(
            DB::PAYMENT_ID_TO_TX_HASH_PREFIX, std::make_pair(
            paymentId, totalTxsCountForPaymentId - 1
        ), transactionHash
        ));
    return *this;
}
...
BlockchainWriteBatch &BlockchainWriteBatch::insertRawBlock(
    uint32_t blockIndex,
...
BlockchainWriteBatch &BlockchainWriteBatch::insertClosestTimestampBlockIndex(
    uint64_t timestamp,
    uint32_t blockIndex
)
{
...
BlockchainWriteBatch &BlockchainWriteBatch::insertTimestamp(
    uint64_t timestamp,
...
BlockchainWriteBatch &BlockchainWriteBatch::removeSpentKeyImages(
    uint32_t blockIndex,
...
BlockchainWriteBatch &BlockchainWriteBatch::removeCachedTransaction(
    const Crypto::Hash &transactionHash,
    uint64_t totalTxsCount
)
{
...
BlockchainWriteBatch &BlockchainWriteBatch::removePaymentId(
    const Crypto::Hash paymentId,
    uint32_t totalTxsCountForPaymentId
)
{
...
BlockchainWriteBatch &BlockchainWriteBatch::removeCachedBlock(
...
)
{
    ...
    rawDataToInsert.emplace_back(
        DB::serialize(
            DB::BLOCK_INDEX_TO_BLOCK_HASH_PREFIX, DB::LAST_BLOCK_INDEX_KEY, blockIndex - 1
        ));
    return *this;
}
...
)
{
    ...
    for (uint32_t i = 0; i < outputsToRemoveCount; ++i)
    {
        rawKeysToRemove.emplace_back(
            DB::serializeKey(
                DB::KEY_OUTPUT_AMOUNT_PREFIX, std::make_pair(
                amount, totalOutputsCountForAmount + i
            )));
    }
    return *this;
}
...
std::vector<
    std::pair<
        std::string, std::string>> BlockchainWriteBatch::extractRawDataToInsert()
{
