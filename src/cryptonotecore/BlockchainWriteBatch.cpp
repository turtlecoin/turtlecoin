                                      ...
                                      uint32_t totalTxsCountForPaymentId)
{
    ...
    rawDataToInsert.emplace_back(DB::serialize(DB::PAYMENT_ID_TO_TX_HASH_PREFIX, std::make_pair(paymentId,
                                                                                                totalTxsCountForPaymentId -
                                                                                                1), transactionHash));
    return *this;
}
...
BlockchainWriteBatch &BlockchainWriteBatch::removeCachedBlock(const Crypto::Hash &blockHash, uint32_t blockIndex)
{
    ...
    rawDataToInsert.emplace_back(DB::serialize(DB::BLOCK_INDEX_TO_BLOCK_HASH_PREFIX, DB::LAST_BLOCK_INDEX_KEY,
                                               blockIndex - 1));
    return *this;
}
                                                   ...
                                                   uint32_t totalOutputsCountForAmount)
{
    ...
    {
        rawKeysToRemove.emplace_back(DB::serializeKey(DB::KEY_OUTPUT_AMOUNT_PREFIX, std::make_pair(amount,
                                                                                                   totalOutputsCountForAmount +
                                                                                                   i)));
    }
    return *this;
}
