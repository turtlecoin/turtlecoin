...
namespace CryptoNote
{
    ...
    static_assert(
        CryptoNote::UpgradeDetectorBase::UNDEF_HEIGHT ==
        UINT32_C(0xFFFFFFFF), "UpgradeDetectorBase::UNDEF_HEIGHT has invalid value"
    );
        ...
        public:
                ...
                if (upgradeHeight == UNDEF_HEIGHT)
                {
                    ...
                    else if (m_targetVersion - 1 == m_blockchain.back().bl.majorVersion)
                    {
                        m_votingCompleteHeight =
                            findVotingCompleteHeight(static_cast<uint32_t>(m_blockchain.size() - 1));

                    }
                    else if (m_targetVersion <= m_blockchain.back().bl.majorVersion)
                    {
                        auto it = std::lower_bound(
                            m_blockchain.begin(), m_blockchain.end(), m_targetVersion, [](
                    ...
                    if (m_blockchain.size() <= upgradeHeight + 1)
                    {
                        if (m_blockchain.back().bl.majorVersion >= m_targetVersion)
                        {
                            logger(Logging::ERROR, Logging::BRIGHT_RED) << "Internal error: block at height "
                                                                        << (m_blockchain.size() - 1)
                                                                        << " has invalid version "
                                                                        << static_cast<int>(m_blockchain.back().bl
                                                                                                        .majorVersion)
                                                                        << ", expected "
            ...
            void blockPopped()
            {
                if (m_votingCompleteHeight != UNDEF_HEIGHT)
                {
                    ...
                    {
                        logger(Logging::TRACE, Logging::BRIGHT_YELLOW) << "###### UPGRADE after block index "
            ...
            size_t getNumberOfVotes(uint32_t height)
            {
                ...
                for (size_t i = height + 1 - m_currency.upgradeVotingWindow(); i <= height; ++i)
                {
                    ...
                    voteCounter += (b.majorVersion == m_targetVersion - 1) && (b.minorVersion == BLOCK_MINOR_VERSION_1)
                                   ? 1
                                   : 0;
                }
