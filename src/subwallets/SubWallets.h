...
class SubWallets
{
    public:
        ...
        std::vector<
            std::tuple<
                std::string, uint64_t, uint64_t>> getBalances(
            const uint64_t currentHeight
        ) const;
    ...
    private:
        ...
        std::unordered_map<
            Crypto::Hash, Crypto::SecretKey
        > m_transactionPrivateKeys;

        /* A mapping of key images to the subwallet public spend key that owns them */
        std::unordered_map<
            Crypto::KeyImage, Crypto::PublicKey
        > m_keyImageOwners;
