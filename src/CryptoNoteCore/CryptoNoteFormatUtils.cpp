...
bool get_tx_fee(const Transaction& tx, uint64_t & fee) {
  ...
  for (const auto& in : tx.inputs) {
    if (mpark::holds_alternative<KeyInput>(in)) {
      amount_in += mpark::get<KeyInput>(in).amount;
    }
  }
...
bool checkOutsValid(const TransactionPrefix& tx, std::string* error) {
  for (const TransactionOutput& out : tx.outputs) {
    if ( mpark::holds_alternative<KeyOutput>(out.target) ) {
      if (out.amount == 0) {
        if (error) {
      ...
      if (!check_key(mpark::get<KeyOutput>(out.target).key)) {
        if (error) {
...
bool checkInputsOverflow(const TransactionPrefix &tx) {
  ...
  for (const auto &in : tx.inputs) {
    uint64_t amount = 0;
    if(mpark::holds_alternative<KeyInput>(in)) {
      amount = mpark::get<KeyInput>(in).amount;
    }
