...
void DatabaseBlockchainCache::requestDeletePaymentId(BlockchainWriteBatch &writeBatch, const Crypto::Hash &paymentId,
                                                     size_t toDelete)
{
...
ExtractOutputKeysResult DatabaseBlockchainCache::extractKeyOutputKeys(uint64_t amount,
...
DatabaseBlockchainCache::extractKeyOtputReferences(uint64_t amount, Common::ArrayView<uint32_t> globalIndexes,
...
std::vector<uint64_t> DatabaseBlockchainCache::getLastUnits(size_t count, uint32_t blockIndex, UseGenesis useGenesis,
...
std::vector<RawBlock> DatabaseBlockchainCache::getBlocksByHeight(const uint64_t startHeight, uint64_t endHeight) const
{
...
void DatabaseBlockchainCache::setParent(IBlockchainCache *ptr) { assert(false); }
...
void DatabaseBlockchainCache::addGenesisBlock(CachedBlock &&genesisBlock)
{
    ...
    CachedBlockInfo blockInfo{genesisBlock.getBlockHash(),  genesisBlock.getBlock().timestamp, 1, minerReward, 1,
