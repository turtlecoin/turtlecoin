...
namespace CryptoNote {
  ...
  void TransactionImpl::signInputKey(size_t index, const TransactionTypes::InputKeyInfo& info, const KeyPair& ephKeys) {
    const auto& input = mapbox::util::get<KeyInput>(getInputChecked(transaction, index, TransactionTypes::InputType::Key));
  ...
  void TransactionImpl::getInput(size_t index, KeyInput& input) const {
    input = mapbox::util::get<KeyInput>(getInputChecked(transaction, index, TransactionTypes::InputType::Key));
  }
