...
class TransactionImpl : public ITransaction
{
  public:
    ...
    virtual void signInputKey(size_t input, const TransactionTypes::InputKeyInfo &info,
...
std::unique_ptr<ITransaction> createTransaction() { return std::unique_ptr<ITransaction>(new TransactionImpl()); }
...
uint64_t TransactionImpl::getUnlockTime() const { return transaction.unlockTime; }
...
BinaryArray TransactionImpl::getTransactionData() const { return toBinaryArray(transaction); }
...
BinaryArray TransactionImpl::getExtra() const { return transaction.extra; }

size_t TransactionImpl::getInputCount() const { return transaction.inputs.size(); }
...
size_t TransactionImpl::getOutputCount() const { return transaction.outputs.size(); }
