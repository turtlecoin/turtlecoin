...
BlockchainWriteBatch &BlockchainWriteBatch::insertSpentKeyImages(uint32_t blockIndex,
                                      ...
                                      uint32_t totalTxsCountForPaymentId)
{
    ...
    rawDataToInsert.emplace_back(
            DB::serialize(DB::PAYMENT_ID_TO_TX_HASH_PREFIX, std::make_pair(paymentId, totalTxsCountForPaymentId - 1),
                          transactionHash));
    return *this;
}

BlockchainWriteBatch &BlockchainWriteBatch::insertCachedBlock(const CachedBlockInfo &block, uint32_t blockIndex,
                                                                   ...
                                                                   uint32_t totalKeyOutputAmountsCount)
{
    ...
    rawDataToInsert.emplace_back(DB::serialize(DB::KEY_OUTPUT_AMOUNTS_COUNT_PREFIX, DB::KEY_OUTPUT_AMOUNTS_COUNT_KEY,
...
BlockchainWriteBatch &BlockchainWriteBatch::removeKeyOutputAmounts(uint32_t keyOutputAmountsToRemoveCount,
                                                                   uint32_t totalKeyOutputAmountsCount)
{
    ...
    {
        rawKeysToRemove.emplace_back(
                DB::serializeKey(DB::KEY_OUTPUT_AMOUNTS_COUNT_PREFIX, totalKeyOutputAmountsCount + i));
    }
