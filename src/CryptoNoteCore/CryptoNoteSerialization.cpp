...
uint64_t getSignaturesCount(const TransactionInput &input)
{
  ...
  return mpark::visit(txin_signature_size_visitor(), input);
}
...
void serialize(Transaction &tx, ISerializer &serializer)
{
  ...
  if (serializer.type() == ISerializer::INPUT && !(sigSize == 1 && mpark::holds_alternative<BaseInput>(tx.inputs[0])))
  {
...
void serialize(TransactionOutputTarget &output, ISerializer &serializer)
{
  if (serializer.type() == ISerializer::OUTPUT)
  {
    ...
    uint8_t tag = mpark::visit(tagGetter, output);

    serializer.binary(&tag, sizeof(tag), "type");

    VariantSerializer visitor(serializer, "data");

    mpark::visit(visitor, output);
  }
  else
  {
    uint8_t tag;
