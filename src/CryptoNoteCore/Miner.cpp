...
namespace CryptoNote
{
    ...
    bool miner::init(const MinerConfig &config)
    {
        if (!config.extraMessages.empty())
        {
            ...
            std::string filebuf;
            if (Common::loadFileToString(m_config_folder_path + "/" + CryptoNote::parameters::MINER_CONFIG_FILE_NAME,
                                         filebuf))
            {
        ...
        while (!m_stop)
        {
            ...
            if (!m_stop && check_hash(h, local_diff))
            {
                ...
                } else
                {
                    ...
                    Common::saveStringToFile(
                            m_config_folder_path + "/" + CryptoNote::parameters::MINER_CONFIG_FILE_NAME,
