...
BlockchainCache::BlockchainCache(const std::string &filename, const Currency &currency,
                                 std::shared_ptr<Logging::ILogger> logger_, IBlockchainCache *parent,
...
std::vector<uint32_t> BlockchainCache::getRandomOutsByAmount(Amount amount, size_t count, uint32_t blockIndex) const
{
    ...
    /* Starting from the end of the outputs vector, return the first output
       that is unlocked */
    const auto end = std::find_if(outs.rbegin(), outs.rend(),
                                  [&](const auto index) {
...
ExtractOutputKeysResult
BlockchainCache::extractKeyOutputs(uint64_t amount, uint32_t blockIndex, Common::ArrayView<uint32_t> globalIndexes,
...
std::vector<uint64_t> BlockchainCache::getLastTimestamps(size_t count, uint32_t blockIndex, UseGenesis useGenesis) const
{
...
uint64_t BlockchainCache::getDifficultyForNextBlock(uint32_t blockIndex) const
{
    ...
    auto timestamps = getLastTimestamps(currency.difficultyBlocksCountByBlockVersion(nextBlockMajorVersion, blockIndex),
                                        blockIndex, skipGenesisBlock);
    auto commulativeDifficulties = getLastCumulativeDifficulties(
        currency.difficultyBlocksCountByBlockVersion(nextBlockMajorVersion, blockIndex), blockIndex, skipGenesisBlock);
...
uint64_t BlockchainCache::getAlreadyGeneratedCoins() const { return getAlreadyGeneratedCoins(getTopBlockIndex()); }
