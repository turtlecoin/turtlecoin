...
namespace CryptoNote
{
    ...
    namespace
    {
        ...
        void splitGlobalIndexes(T &sourceContainer, T &destinationContainer, uint32_t splitBlockIndex,
                                F lowerBoundFunction)
        {
            for (auto it = sourceContainer.begin(); it != sourceContainer.end();)
            {
                ...
                std::move(newCacheOutputsIteratorStart, it->second.outputs.end(),
    ...
    BlockchainCache::BlockchainCache(const std::string &filename, const Currency &currency, Logging::ILogger &logger_,
            ...
            : filename(filename), currency(currency), logger(logger_, "BlockchainCache"), parent(parent),
              storage(new BlockchainStorage(100))
    {
        if (parent == nullptr)
        {
            ...
            doPushBlock(genesisBlock, transactions, validatorState, coinbaseTransactionSize, minerReward, 1,
    ...
    PushedBlockInfo BlockchainCache::getPushedBlockInfo(uint32_t blockIndex) const
    {
        ...
        } else
        {
            ...
            {
                Difficulty cumulativeDifficulty = parent->getLastCumulativeDifficulties(1, startIndex - 1,
                                                                                        addGenesisBlock)[0];
...
// split blockchain near its top.
    ...
    void BlockchainCache::splitKeyOutputsGlobalIndexes(BlockchainCache &newCache, uint32_t splitBlockIndex)
    {
        ...
        splitGlobalIndexes(keyOutputsGlobalIndexes, newCache.keyOutputsGlobalIndexes, splitBlockIndex,
                           lowerBoundFunction);
    ...
    uint32_t
    BlockchainCache::insertKeyOutputToGlobalIndex(uint64_t amount, PackedOutIndex output, uint32_t blockIndex)
    {
...
//  assert(blockInfos.get<BlockHashTag>().count(blockHash) > 0);
    ...
    size_t BlockchainCache::getKeyOutputsCountForAmount(uint64_t amount, uint32_t blockIndex) const
    {
        ...
        auto lowerBound = std::lower_bound(it->second.outputs.begin(), it->second.outputs.end(), blockIndex,
    ...
    void BlockchainCache::serialize(ISerializer &s)
    {
        ...
        } else
        {
            ...
            readSequence<CachedTransactionInfo>(std::inserter(restoredTransactions, restoredTransactions.end()),
                                                "transactions", s);
            readSequence<SpentKeyImage>(std::inserter(restoredSpentKeyImages, restoredSpentKeyImages.end()),
                                        "spent_key_images", s);
    ...
    std::vector<uint32_t>
    BlockchainCache::getRandomOutsByAmount(Amount amount, size_t count, uint32_t blockIndex) const
    {
                                               ...
                                               std::vector<std::pair<Crypto::Hash, size_t>> &outputReferences) const
    {
        ...
        return extractKeyOutputs(amount, getTopBlockIndex(), globalIndexes,
                                 [&](const CachedTransactionInfo &info, PackedOutIndex index, uint32_t globalIndex)
                                 {
                                     outputReferences.push_back(
                                             std::make_pair(info.transactionHash, index.outputIndex));
                                     return ExtractOutputKeysResult::SUCCESS;
                                 });
    }
...
// TODO: start from index
    ExtractOutputKeysResult BlockchainCache::extractKeyOutputs(
            ...
            std::function<ExtractOutputKeysResult(const CachedTransactionInfo &info, PackedOutIndex index,
                                                  uint32_t globalIndex)> pred) const
    {
        assert(!globalIndexes.isEmpty());
        assert(std::is_sorted(globalIndexes.begin(), globalIndexes.end()));                            // sorted
        assert(std::adjacent_find(globalIndexes.begin(), globalIndexes.end()) == globalIndexes.end()); // unique
        ...
        if (parentIndexesIterator != globalIndexes.begin())
        {
            ...
            auto result = parent->extractKeyOutputs(amount, blockIndex,
                                                    globalIndexes.head(parentIndexesIterator - globalIndexes.begin()),
                                                    pred);
                                                        ...
                                                        std::function<uint64_t(const CachedBlockInfo &)> pred) const
    {
        ...
        {
            result =
                    parent->getLastUnits(count - realCount, std::min(blockIndex, parent->getTopBlockIndex()),
                                         useGenesis, pred);
        }

        std::transform(std::next(blocksIndex.begin(), from), std::next(blocksIndex.begin(), to),
    ...
    Difficulty BlockchainCache::getDifficultyForNextBlock(uint32_t blockIndex) const
    {
        ...
        auto timestamps = getLastTimestamps(
                currency.difficultyBlocksCountByBlockVersion(nextBlockMajorVersion, blockIndex), blockIndex,
                skipGenesisBlock);
        auto commulativeDifficulties =
                getLastCumulativeDifficulties(
                        currency.difficultyBlocksCountByBlockVersion(nextBlockMajorVersion, blockIndex), blockIndex,
                        skipGenesisBlock);
