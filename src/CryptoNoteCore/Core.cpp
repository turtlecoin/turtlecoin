...
Core::Core(const Currency &currency, std::shared_ptr<Logging::ILogger> logger, Checkpoints &&checkpoints,
    ...
    : currency(currency), dispatcher(dispatcher), contextGroup(dispatcher), logger(logger, "Core"),
      checkpoints(std::move(checkpoints)), upgradeManager(new UpgradeManager()),
      blockchainCacheFactory(std::move(blockchainCacheFactory)), mainChainStorage(std::move(mainchainStorage)),
      initialized(false)
{
...
bool Core::queryBlocksDetailed(const std::vector<Crypto::Hash> &knownBlockHashes, uint64_t timestamp,
                               ...
                               std::vector<BlockDetails> &entries, uint32_t blockCount) const
{
    assert(entries.empty());
    assert(!chainsLeaves.empty());
    assert(!chainsStorage.empty());
...
bool Core::getTransactionsStatus(std::unordered_set<Crypto::Hash> transactionHashes,
...
bool Core::getWalletSyncData(const std::vector<Crypto::Hash> &knownBlockHashes, const uint64_t startHeight,
                             const uint64_t startTimestamp, const uint64_t blockCount,
                             std::vector<WalletTypes::WalletBlockInfo> &walletBlocks) const
{
    ...
    try
    {
        ...
        uint64_t startIndex = std::max(
            ...
            lastKnownBlockHashHeight == 0 ? 0 : lastKnownBlockHashHeight + 1, firstBlockHeight);
...
std::error_code Core::addBlock(const CachedBlock &cachedBlock, RawBlock &&rawBlock)
{
    ...
    if (addOnTop)
    {
        ...
        {
            // add block on top of segment which is not leaf! the case when we got more than one alternative block on
            // the same height
            ...
            newlyForkedChainPtr->pushBlock(cachedBlock, transactions, validatorState, cumulativeBlockSize,
                                           emissionChange, currentDifficulty, std::move(rawBlock));
...
bool Core::getTransactionGlobalIndexes(const Crypto::Hash &transactionHash, std::vector<uint32_t> &globalIndexes) const
{
    throwIfNotInitialized();
    IBlockchainCache *segment = chainsLeaves[0];
...
bool Core::getGlobalIndexesForRange(const uint64_t startHeight, const uint64_t endHeight,
                            ...
                            uint64_t &difficulty, uint32_t &height) const
{
    ...
    */
    ...
    bool r = currency.constructMinerTx(b.majorVersion, height, medianSize, alreadyGeneratedCoins, transactionsSize, fee,
    ...
    {
        r = currency.constructMinerTx(b.majorVersion, height, medianSize, alreadyGeneratedCoins, cumulativeSize, fee,
                                      adr, b.baseTransaction, extraNonce, 11);
        if (!r)
        {
        ...
        if (coinbaseBlobSize < cumulativeSize - transactionsSize)
        {
            ...
            // here  could be 1 byte difference, because of extra field counter is varint, and it can become from 1-byte
            // len to 2-bytes len.
    ...
    for (const auto &input : transaction.inputs)
    {
        if (input.type() == typeid(KeyInput))
        {
            ...
            if (!checkpoints.isInCheckpointZone(blockIndex + 1))
            {
                ...
                if (!Crypto::crypto_ops::checkRingSignature(cachedTransaction.getTransactionPrefixHash(), in.keyImage,
                                                            outputKeys, transaction.signatures[inputIndex]))
                {
...
std::error_code Core::validateBlock(const CachedBlock &cachedBlock, IBlockchainCache *cache, uint64_t &minerReward)
{
    ...
    auto timestamps = cache->getLastTimestamps(currency.timestampCheckWindow(previousBlockIndex + 1),
...
const Currency &Core::getCurrency() const { return currency; }
...
bool Core::validateBlockTemplateTransaction(const CachedTransaction &cachedTransaction,
...
void Core::fillBlockTemplate(BlockTemplate &block, const size_t medianSize, const size_t maxCumulativeSize,
                             const uint64_t height, size_t &transactionsSize, uint64_t &fee) const
{
