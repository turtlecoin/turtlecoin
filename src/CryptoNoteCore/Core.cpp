...
namespace CryptoNote
{
    ...
    namespace
    {
                                  ...
                                  const CachedBlock &cachedBlock, uint64_t cumulativeSize, uint64_t cumulativeFee)
        {
            ...
            auto lastBlocksSizes = segment.getLastBlocksSizes(currency.rewardBlocksWindow(), previousBlockIndex,
                                                              addGenesisBlock);
            ...
            if (!currency.getBlockReward(cachedBlock.getBlock().majorVersion, blocksSizeMedian,
                                         cumulativeSize, alreadyGeneratedCoins, cumulativeFee, reward,
                                         emissionChange))
            {
    ...
    Core::Core(const Currency &currency, Logging::ILogger &logger, Checkpoints &&checkpoints,
            ...
            : currency(currency), dispatcher(dispatcher), contextGroup(dispatcher), logger(logger, "Core"),
                           ...
                           uint32_t &currentIndex, uint32_t &fullOffset, std::vector<BlockFullInfo> &entries) const
    {
        ...
        try
        {
            ...
            size_t hashesPushed = pushBlockHashes(startIndex, fullOffset, BLOCKS_IDS_SYNCHRONIZING_DEFAULT_COUNT,
                                                  entries);
    ...
    bool
    Core::queryBlocksLite(const std::vector<Crypto::Hash> &knownBlockHashes, uint64_t timestamp, uint32_t &startIndex,
    ...
    void
    Core::getTransactions(const std::vector<Crypto::Hash> &transactionHashes, std::vector<BinaryArray> &transactions,
                          std::vector<Crypto::Hash> &missedHashes) const
    {
        assert(!chainsLeaves.empty());
        assert(!chainsStorage.empty());
...
// TODO: just use mainChain->getDifficultyForNextBlock() ?
    ...
    std::error_code Core::addBlock(const CachedBlock &cachedBlock, RawBlock &&rawBlock)
    {
        ...
        {
            uint64_t fee = 0;
            auto transactionValidationResult = validateTransaction(transaction, validatorState, cache, fee,
                                                                   previousBlockIndex);
        ...
        auto lastBlocksSizes = cache->getLastBlocksSizes(currency.rewardBlocksWindow(), previousBlockIndex,
                                                         addGenesisBlock);
        auto blocksSizeMedian = Common::medianValue(lastBlocksSizes);

        if (!currency.getBlockReward(cachedBlock.getBlock().majorVersion, blocksSizeMedian,
                                     cumulativeBlockSize, alreadyGeneratedCoins, cumulativeFee, reward,
                                     emissionChange))
        {
            logger(Logging::DEBUGGING) << "Block " << blockStr << " has too big cumulative size";
            return error::BlockValidationError::CUMULATIVE_BLOCK_SIZE_TOO_BIG;
        ...
        if (addOnTop)
        {
                ...
                } else
                {
                    ...
                    if (cache->getCurrentCumulativeDifficulty() > mainChainCache->getCurrentCumulativeDifficulty())
                    {
                        size_t endpointIndex =
                                std::distance(chainsLeaves.begin(),
                                              std::find(chainsLeaves.begin(), chainsLeaves.end(), cache));
            ...
            } else
            {
                ...
                newlyForkedChainPtr->pushBlock(cachedBlock, transactions, validatorState, cumulativeBlockSize,
                                               emissionChange,
                                               currentDifficulty, std::move(rawBlock));
    ...
    bool Core::isTransactionValidForPool(const CachedTransaction &cachedTransaction,
                                         TransactionValidatorState &validatorState)
    {
        ...
        if (auto validationResult = validateTransaction(cachedTransaction, validatorState, chainsLeaves[0], fee,
                                                        getTopBlockIndex()))
        {
            logger(Logging::DEBUGGING) << "Transaction " << cachedTransaction.getTransactionHash()
        ...
        bool isFusion = fee == 0 && currency.isFusionTransaction(cachedTransaction.getTransaction(),
                                                                 cachedTransaction.getTransactionBinaryArray().size());
                                ...
                                Difficulty &difficulty, uint32_t &height) const
    {
        ...
        */
        ...
        bool r = currency.constructMinerTx(b.majorVersion, height, medianSize, alreadyGeneratedCoins, transactionsSize,
                                           fee, adr,
        ...
        {
            r = currency.constructMinerTx(b.majorVersion, height, medianSize, alreadyGeneratedCoins, cumulativeSize,
                                          fee, adr,
                                          b.baseTransaction, extraNonce, 11);
            if (!r)
            {
            ...
            if (coinbaseBlobSize < cumulativeSize - transactionsSize)
            {
                ...
                if (cumulativeSize != transactionsSize + getObjectBinarySize(b.baseTransaction))
                {
                    ...
                    logger(Logging::DEBUGGING, Logging::BRIGHT_GREEN)
                            << "Setting extra for block: " << b.baseTransaction.extra.size() << ", try_count="
                            << tryCount;
                }
            }
            if (!(cumulativeSize == transactionsSize + getObjectBinarySize(b.baseTransaction)))
    ...
    std::error_code
    Core::validateTransaction(const CachedTransaction &cachedTransaction, TransactionValidatorState &state,
        ...
        for (const auto &input : transaction.inputs)
        {
            if (input.type() == typeid(KeyInput))
            {
                ...
                if (!checkpoints.isInCheckpointZone(blockIndex + 1))
                {
                    ...
                    auto result = cache->extractKeyOutputKeys(in.amount, blockIndex,
                                                              {globalIndexes.data(), globalIndexes.size()}, outputKeys);
                    ...
                    std::for_each(outputKeys.begin(), outputKeys.end(),
                                  [&outputKeyPointers](const Crypto::PublicKey &key)
                                  {
                                      outputKeyPointers.push_back(&key);
                                  });
    ...
    std::error_code
    Core::validateBlock(const CachedBlock &cachedBlock, IBlockchainCache *cache, uint64_t &minerReward)
    {
        ...
        auto timestamps = cache->getLastTimestamps(currency.timestampCheckWindow(), previousBlockIndex,
                                                   addGenesisBlock);
        ...
        for (uint32_t i = commonIndex + 1; i < blockCount; ++i)
        {
            ...
            if (blockTemplate.previousBlockHash != previousBlockHash)
            {
                logger(Logging::ERROR) << "Corrupted blockchain. Block with index " << i << " and hash "
                                       ...
                                       << " has previous block hash " << blockTemplate.previousBlockHash
                                       << ", but parent has hash " << previousBlockHash
            ...
            uint64_t cumulativeFee = std::accumulate(transactions.begin(), transactions.end(), UINT64_C(0),
                                                     [](uint64_t fee, const CachedTransaction &transaction)
                                                     {
                                                         return fee + transaction.getTransactionFee();
                                                     });

            int64_t emissionChange = getEmissionChange(currency, *chainsLeaves[0], i - 1, cachedBlock, cumulativeSize,
                                                       cumulativeFee);
...
//TODO: decompose these two methods
                                       ...
                                       std::vector<BlockShortInfo> &entries) const
    {
        ...
        uint32_t fullBlocksCount = static_cast<uint32_t>(std::min(static_cast<uint32_t>(maxItemsCount),
                                                                  currentIndex - fullOffset + 1));
        entries.reserve(entries.size() + fullBlocksCount);
        ...
        for (uint32_t blockIndex = fullOffset; blockIndex < fullOffset + fullBlocksCount; ++blockIndex)
        {
            ...
            for (auto &rawTransaction : rawBlock.transactions)
            {
                ...
                prefixInfo.txHash =
                        getBinaryArrayHash(
                                rawTransaction); // TODO: is there faster way to get hash without calculation?
    ...
    void Core::deleteLeaf(size_t leafIndex)
    {
        ...
        auto segmentIt =
                std::find_if(chainsStorage.begin(), chainsStorage.end(),
                             [&leaf](const std::unique_ptr<IBlockchainCache> &segment)
                             {
    ...
    void Core::mergeMainChainSegments()
    {
        ...
        auto rootIt = std::find_if(
                ...
                [&rootSegment](const std::unique_ptr<IBlockchainCache> &segment)
                {
    ...
    BlockDetails Core::getBlockDetails(const Crypto::Hash &blockHash) const
    {
        ...
        {
            auto lastBlocksSizes = segment->getLastBlocksSizes(currency.rewardBlocksWindow(), blockDetails.index - 1,
                                                               addGenesisBlock);
        ...
        int64_t emissionChange = 0;
        bool result = currency.getBlockReward(blockDetails.majorVersion, blockDetails.sizeMedian, 0,
        ...
        result = currency.getBlockReward(blockDetails.majorVersion, blockDetails.sizeMedian,
                                                   ...
                                                   bool foundInPool) const
    {
        ...
        } else
        {
            ...
            transactionDetails.size = transactionPool->getTransaction(
                    transactionHash).getTransactionBinaryArray().size();
        ...
        for (size_t i = 0; i < transaction->getInputCount(); ++i)
        {
            ...
            } else if (transaction->getInputType(i) == TransactionTypes::InputType::Key)
            {
                ...
                std::vector<uint32_t> globalIndexes = relativeOutputOffsetsToAbsolute(
                        txInToKeyDetails.input.outputIndexes);
                ExtractOutputKeysResult result = segment->extractKeyOtputReferences(txInToKeyDetails.input.amount,
                                                                                    {globalIndexes.data(),
                                                                                     globalIndexes.size()},
                                                                                    outputReferences);
                if (result == result)
    ...
    void Core::transactionPoolCleaningProcedure()
    {
        ...
        try
        {
            for (;;)
            {
                ...
                notifyObservers(makeDelTransactionMessage(std::move(deletedTransactions),
                                                          Messages::DeleteTransaction::Reason::Outdated));
            }
    ...
    void Core::updateBlockMedianSize()
    {
        ...
        blockMedianSize = std::max(Common::medianValue(lastBlockSizes),
                                   static_cast<uint64_t>(nextBlockGrantedFullRewardZone));
    }
