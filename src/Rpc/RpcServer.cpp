...
RpcServer::RpcServer(System::Dispatcher &dispatcher, std::shared_ptr<Logging::ILogger> log, Core &c, NodeServer &p2p,
...
{
    if (!m_core.getTransactionsStatus(req.transactionHashes, res.transactionsInPool, res.transactionsInBlock,
                                      res.transactionsUnknown))
    {
        res.status = "Failed to perform query";
        return false;
    }
