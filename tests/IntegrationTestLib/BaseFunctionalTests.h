...
namespace Tests
{
    namespace Common
    {
        ...
        public:
            ...
            {
                std::unique_lock<std::mutex> lck(mtx);
                auto result = cv.wait_for(lck, rel_time, [this]()
                { return available; });
                available = false;
                return result;
            }
        ...
        public:
            ...
            void init(po::options_description &desc)
            {
                desc.add_options()
                        ...
                        ("add-daemons,a", po::value<std::vector<std::string>>()->multitoken(),
                         "add daemon to topology");
            }
        ...
        {
        public:
            BaseFunctionalTests(const CryptoNote::Currency &currency, System::Dispatcher &d,
                                const BaseFunctionalTestsConfig &config) :
                    m_dispatcher(d),
                    m_currency(currency),
        ...
        protected:
            ...
            bool
            makeWallet(std::unique_ptr<CryptoNote::IWalletLegacy> &wallet, std::unique_ptr<CryptoNote::INode> &node,
            ...
            bool getNodeTransactionPool(size_t nodeIndex, CryptoNote::INode &node,
