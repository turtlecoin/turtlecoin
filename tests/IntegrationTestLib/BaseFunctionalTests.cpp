...
void BaseFunctionalTests::startNode(size_t index)
{
    ...
    boost::filesystem::path daemonPath = index < m_config.daemons.size() ?
                                         boost::filesystem::path(m_config.daemons[index]) : (
                                                 boost::filesystem::path(m_daemonDir) / DAEMON_FILENAME);
...
namespace
{
    ...
    public:
        WaitForCoinBaseObserver(Semaphore &gotReward, CryptoNote::IWalletLegacy &wallet) : m_gotReward(gotReward),
                                                                                           m_wallet(wallet)
        {}
...
bool
BaseFunctionalTests::mineBlocks(TestNode &node, const CryptoNote::AccountPublicAddress &address, size_t blockCount)
{
...
bool BaseFunctionalTests::makeWallet(std::unique_ptr<CryptoNote::IWalletLegacy> &wallet,
                                     std::unique_ptr<CryptoNote::INode> &node, const std::string &password)
{
...
namespace
{
    struct PeerCountWaiter : CryptoNote::INodeObserver
    {
        ...
        PeerCountWaiter(System::Dispatcher &dispatcher) : m_dispatcher(dispatcher), m_event(m_dispatcher),
...
namespace
{
    struct PoolUpdateWaiter : public INodeObserver
    {
        ...
        PoolUpdateWaiter(System::Dispatcher &dispatcher, System::Event &event) : m_dispatcher(dispatcher),
                                                                                 m_event(event)
        {
        }
                                                 ...
                                                 std::vector<std::unique_ptr<CryptoNote::ITransactionReader>> &txPool)
{
    ...
    while (true)
    {
        ...
        node.getPoolSymmetricDifference(std::vector<Crypto::Hash>(), tailBlockId, isTailBlockActual, addedTxs,
                                        deletedTxsIds,
