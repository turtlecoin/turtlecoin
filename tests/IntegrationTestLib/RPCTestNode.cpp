...
namespace Tests
{
    ...
    bool RPCTestNode::getBlockTemplate(const std::string &minerAddress, CryptoNote::BlockTemplate &blockTemplate,
                                       uint64_t &difficulty)
    {
    ...
    {
        std::unique_ptr<CryptoNote::INode> newNode(
                new CryptoNote::NodeRpcProxy("127.0.0.1", m_rpcPort, m_logger.getLogger()));
        NodeCallback cb;
