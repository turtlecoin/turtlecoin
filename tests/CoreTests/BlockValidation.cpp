...
namespace
{
    ...
    {
        CryptoNote::Difficulty commulative_diffic = cummulative_difficulties.empty() ? 0
                                                                                     : cummulative_difficulties.back();
...
bool gen_block_ts_not_checked::generate(std::vector<test_event_entry> &events) const
{
    ...
    generator.constructBlockManually(blk_1, blk_0r, miner_account,
                                     test_generator::bf_major_ver | test_generator::bf_timestamp, m_blockMajorVersion,
                                     0,
...
bool gen_block_invalid_prev_id::generate(std::vector<test_event_entry> &events) const
{
    ...
    generator.constructBlockManually(blk_1, blk_0, miner_account,
                                     test_generator::bf_major_ver | test_generator::bf_prev_id, m_blockMajorVersion, 0,
                                     0,
                                     prev_id);
    events.push_back(blk_1);
...
bool gen_block_invalid_nonce::generate(std::vector<test_event_entry> &events) const
{
    ...
    if (!lift_up_difficulty(*m_currency, events, timestamps, commulative_difficulties, generator, 2, blk_0,
                            miner_account,
                            m_blockMajorVersion))
    {
        return false;
        ...
        : m_corrupt_blocks_begin_idx(0), m_blockMajorVersion(blockMajorVersion)
{
    ...
    currencyBuilder.upgradeHeightV2(
            blockMajorVersion == CryptoNote::BLOCK_MAJOR_VERSION_1 ? IUpgradeDetector::UNDEF_HEIGHT : UINT32_C(0));
