...
bool gen_ring_signature_2::generate(std::vector<test_event_entry> &events) const
{
    ...
    MAKE_TX_MIX(events, tx_0, bob_account, alice_account, MK_COINS(244) - m_currency->minimumFee(), 3,
                blk_4); // 12 + 2N
    MAKE_NEXT_BLOCK_TX1(events, blk_5, blk_4r, miner_account,
                        tx_0);                                           // 13 + 2N
    DO_CALLBACK(events,
                "check_balances_2");                                                                   // 14 + 2N
...
bool gen_ring_signature_big::generate(std::vector<test_event_entry> &events) const
{
    ...
    for (size_t i = 0; i < m_test_size; ++i)
    {
        ...
        {
            MAKE_TX_LIST(events, txs_blk_i, miner_account, accounts[i], m_currency->minimumFee(),
                         blk_with_unlocked_out);
        }
