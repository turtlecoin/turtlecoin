...
namespace
{
                    ...
                    uint8_t majorVersion, uint8_t minorVersion)
    {
        ...
        for (size_t i = 0; i < count; ++i)
        {
            ...
            bool r = generator.constructBlockManually(b, prevBlock, minerAcc,
...
bool gen_upgrade::generate(std::vector<test_event_entry> &events) const
{
    ...
    // Vote for upgrade
    BlockTemplate blk1;
    if (!makeBlocks(events, generator, blk1, blk0, minerAccount, m_currency->minNumberVotingBlocks(),
                    BLOCK_MAJOR_VERSION_1, BLOCK_MINOR_VERSION_1))
    {
        return false;
    }
    ...
    BlockTemplate blk2;
    if (!makeBlocks(events, generator, blk2, blk1, minerAccount,
                    m_currency->upgradeVotingWindow() - m_currency->minNumberVotingBlocks() - 1,
    ...
    BlockTemplate blk3;
    if (!makeBlocks(events, generator, blk3, blk2, minerAccount, upgradeHeight - votingCompleteHeight - 1,
                    BLOCK_MAJOR_VERSION_1, BLOCK_MINOR_VERSION_0))
    {
        return false;
    }
...
bool gen_upgrade::checkBeforeUpgrade(std::vector<test_event_entry> &events, test_generator &generator,
                                     const CryptoNote::BlockTemplate &parentBlock,
                                     ...
                                     bool checkReward) const
{
    ...
    if (checkReward)
    {
        ...
        if (!generator.constructMaxSizeBlock(alternativeBlk, parentBlock, minerAcc, m_currency->rewardBlocksWindow(),
                                             txs))
        {
            return false;
        }
    ...
    return makeBlocks(events, generator, badBlock, parentBlock, minerAcc, 1, BLOCK_MAJOR_VERSION_2,
                      BLOCK_MINOR_VERSION_0);
}

bool gen_upgrade::checkAfterUpgrade(std::vector<test_event_entry> &events, test_generator &generator,
                                    const CryptoNote::BlockTemplate &parentBlock,
                                    const CryptoNote::AccountBase &minerAcc) const
{
    ...
    BlockTemplate badBlock;
    DO_CALLBACK(events, "markInvalidBlock");
    if (!makeBlocks(events, generator, badBlock, parentBlock, minerAcc, 1, BLOCK_MAJOR_VERSION_1,
                    BLOCK_MINOR_VERSION_0))
    {
        return false;
    }
    ...
    DO_CALLBACK(events, "markInvalidBlock");
    return makeBlocks(events, generator, badBlock, parentBlock, minerAcc, 1, BLOCK_MAJOR_VERSION_1,
                      BLOCK_MINOR_VERSION_1);
}

bool gen_upgrade::check_block_verification_context(std::error_code bve, size_t eventIdx,
...
bool gen_upgrade::markInvalidBlock(CryptoNote::Core & /*c*/, size_t evIndex,
...
bool gen_upgrade::checkBlockTemplateVersionIsV1(CryptoNote::Core &c, size_t /*evIndex*/,
                                                const std::vector<test_event_entry> & /*events*/)
{
...
bool gen_upgrade::checkBlockTemplateVersionIsV2(CryptoNote::Core &c, size_t /*evIndex*/,
                                                const std::vector<test_event_entry> & /*events*/)
{
...
bool
gen_upgrade::checkBlockRewardEqFee(CryptoNote::Core &c, size_t evIndex, const std::vector<test_event_entry> &events)
{
...
bool
gen_upgrade::checkBlockRewardIsZero(CryptoNote::Core &c, size_t evIndex, const std::vector<test_event_entry> &events)
{
...
bool gen_upgrade::rememberCoinsInCirculationBeforeUpgrade(CryptoNote::Core &c, size_t /*evIndex*/,
                                                          const std::vector<test_event_entry> & /*events*/)
{
...
bool gen_upgrade::rememberCoinsInCirculationAfterUpgrade(CryptoNote::Core &c, size_t /*evIndex*/,
                                                         const std::vector<test_event_entry> & /*events*/)
{
