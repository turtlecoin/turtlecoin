...
namespace
{
    struct tx_builder
    {
        ...
        void
        step2_fill_inputs(const AccountKeys &sender_account_keys, const std::vector<TransactionSourceEntry> &sources)
        {
            BOOST_FOREACH (const TransactionSourceEntry &src_entr, sources)
                        {
                            ...
                            BOOST_FOREACH (const TransactionSourceEntry::OutputEntry &out_entry,
                                           src_entr.outputs)input_to_key.outputIndexes.push_back(out_entry.first);

                            input_to_key.outputIndexes = absolute_output_offsets_to_relative(
            ...
            BOOST_FOREACH (const TransactionDestinationEntry &dst_entr, destinations)
                        {
                            ...
                            Crypto::generate_key_derivation(dst_entr.addr.viewPublicKey, m_tx_key.secretKey,
                                                            derivation);
                            Crypto::derive_public_key(derivation, output_index, dst_entr.addr.spendPublicKey,
                                                      out_eph_public_key);
            ...
            BOOST_FOREACH (const TransactionSourceEntry &src_entr, sources)
                        {
                            ...
                            BOOST_FOREACH (const TransactionSourceEntry::OutputEntry &o,
                                           src_entr.outputs)
...
bool gen_tx_big_version::generate(std::vector<test_event_entry> &events) const
{
    ...
    std::vector<TransactionSourceEntry> sources;
    std::vector<TransactionDestinationEntry> destinations;
    fill_tx_sources_and_destinations(events, blk_0r, miner_account, miner_account, MK_COINS(1),
...
bool gen_tx_mixed_key_offest_not_exist::generate(std::vector<test_event_entry> &events) const
{
    ...
    std::vector<TransactionSourceEntry> sources;
    std::vector<TransactionDestinationEntry> destinations;
    fill_tx_sources_and_destinations(events, blk_2, bob_account, miner_account, MK_COINS(1), m_currency->minimumFee(),
                                     1,
...
bool gen_tx_key_image_not_derive_from_tx_key::generate(std::vector<test_event_entry> &events) const
{
    ...
    std::vector<TransactionSourceEntry> sources;
    std::vector<TransactionDestinationEntry> destinations;
    fill_tx_sources_and_destinations(events, blk_0f, miner_account, miner_account, MK_COINS(1),
                                     m_currency->minimumFee(),
                                     0, sources, destinations);
...
bool gen_tx_check_input_unlock_time::generate(std::vector<test_event_entry> &events) const
{
    ...
    auto make_tx_to_acc = [&](size_t acc_idx, uint64_t unlock_time)
    {
        txs_0.push_back(make_simple_tx_with_unlock_time(events, blk_1, miner_account, accounts[acc_idx],
                                                        MK_COINS(1) + m_currency->minimumFee(),
                                                        m_currency->minimumFee(),
                                                        unlock_time));
        events.push_back(txs_0.back());
    };
