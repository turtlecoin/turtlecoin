...
namespace concolor
{
    ...
    inline std::basic_ostream<char, std::char_traits<char>> &
    red(std::basic_ostream<char, std::char_traits<char>> &ostr)
    {
    ...
    inline std::basic_ostream<char, std::char_traits<char>> &
    green(std::basic_ostream<char, std::char_traits<char>> &ostr)
    {
...
namespace
{
    ...
    uint64_t getSummaryOutsAmount(const CryptoNote::Transaction &transaction)
    {
        return std::accumulate(transaction.outputs.begin(), transaction.outputs.end(), uint64_t(0),
                               [](uint64_t sum, const CryptoNote::TransactionOutput &out)
                               { return sum + out.amount; });
    }
}
...
namespace CryptoNote
{
    ...
    inline bool operator==(const CryptoNote::Transaction &a, const CryptoNote::Transaction &b)
    {
        ...
        return std::equal(a.signatures.begin(), a.signatures.end(), b.signatures.begin(),
                          [](const SigVect &l, const SigVect &r)
    ...
    inline bool operator==(const CryptoNote::ParentBlock &a, const CryptoNote::ParentBlock &b)
    {
        return std::equal(a.blockchainBranch.begin(), a.blockchainBranch.end(), b.blockchainBranch.end()) &&
               std::equal(a.baseTransactionBranch.begin(), a.baseTransactionBranch.end(),
                          b.baseTransactionBranch.begin()) &&
...
{
    template<class Archive>
    void serialize(Archive &ar, RawBlock &raw, const unsigned int /*version*/)
    {
        ar & raw.block;
        ar & raw.transactions;
    }
    ...
    try
    {
        ...
        CryptoNote::Core c(
                ...
                std::unique_ptr<CryptoNote::IBlockchainCacheFactory>(
                        new CryptoNote::DatabaseBlockchainCacheFactory(database, logger)),
