...
namespace
{
    ...
    uint64_t get_tx_out_amount(const Transaction &tx)
    {
        return std::accumulate(tx.outputs.begin(), tx.outputs.end(), uint64_t(0),
                               [](uint64_t sum, const TransactionOutput &o)
                               { return sum + o.amount; });
    }
}
...
bool gen_block_reward::generate(std::vector<test_event_entry> &events) const
{
    ...
    // Test: block reward == transactions fee
    {
        ...
        bool r = constructMinerTxBySize(*m_currency, miner_tx, blk_7.majorVersion,
                                        CachedBlock(blk_7).getBlockIndex() + 1,
                                        generator.getAlreadyGeneratedCoins(blk_7),
                                        miner_account.getAccountKeys().address,
