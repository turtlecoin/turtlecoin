...
struct output_index
{
    ...
    output_index(const CryptoNote::TransactionOutputTarget &_out, uint64_t _a, size_t _h, size_t tno, size_t ono,
            ...
            : out(_out), amount(_a), blk_height(_h), tx_no(tno), out_no(ono), idx(0), spent(false), p_blk(_pb),
              p_tx(_pt)
    {
    }

    output_index(const output_index &other)
            : out(other.out), amount(other.amount), blk_height(other.blk_height), tx_no(other.tx_no),
              out_no(other.out_no),
    ...
    const std::string toString() const
    {
        ...
        ss << "output_index{blk_height=" << blk_height << " tx_no=" << tx_no << " out_no=" << out_no << " amount="
           << amount
           << " idx=" << idx << " spent=" << spent << "}";
...
namespace
{
    uint64_t get_inputs_amount(const vector<TransactionSourceEntry> &s)
    {
        return std::accumulate(std::begin(s), std::end(s), uint64_t(0),
                               [&](uint64_t sum, const TransactionSourceEntry &e)
                               { return sum + e.amount; });
    }
}
    ...
    for (const auto &blk : blockchain)
    {
        ...
        for (size_t i = 0; i < vtx.size(); i++)
        {
            ...
            for (size_t j = 0; j < tx.outputs.size(); ++j)
            {
                ...
                {
                    output_index oi(out.target, out.amount,
                                    boost::get<BaseInput>(*blk.baseTransaction.inputs.begin()).blockIndex,
                                    i, j, blk, vtx[i]);
