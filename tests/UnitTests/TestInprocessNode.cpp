...
namespace
{
    struct CallbackStatus
    {
        ...
        void setStatus(const std::error_code &ec)
        {
            code = ec;
            waiter.set();
        }
        ...
        std::error_code getStatus() const
        { return code; }
...
void InProcessNodeTests::initNode()
{
    ...
    node.init([&status](std::error_code ec)
              { status.setStatusRemote(ec); });
...
TEST_F(InProcessNodeTests, initOk)
{
    ...
    newNode.init([&status](std::error_code ec)
                 { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.ok());
}
...
TEST_F(InProcessNodeTests, getPeersCountSuccess)
{
    ...
    size_t peerCount = invokeNodeMethod<size_t>([this]()
                                                { return node.getPeerCount(); });
...
TEST_F(InProcessNodeTests, getLastLocalBlockHeightSuccess)
{
    ...
    auto blockHeight = invokeNodeMethod<uint32_t>([this]()
                                                  { return node.getLastLocalBlockHeight(); });
...
TEST_F(InProcessNodeTests, getLastKnownBlockHeightSuccess)
{
    ...
    auto lastKnownBlockHeight = invokeNodeMethod<uint32_t>([this]()
                                                           { return node.getLastKnownBlockHeight() + 1; });
...
TEST_F(InProcessNodeTests, getTransactionOutsGlobalIndicesSuccess)
{
    ...
    uint64_t start = 10;
    std::generate_n(std::back_inserter(expectedIndices), 5, [&start]()
    { return start++; });
    coreStub.set_outputs_gindexs(expectedIndices, true);

    CallbackStatus status(dispatcher);
    node.getTransactionOutsGlobalIndices(ignore, indices,
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsSuccess)
{
    ...
    CallbackStatus status(dispatcher);
    node.getRandomOutsByAmounts({10}, 1, outs, [&status](std::error_code ec)
    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.ok());
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsFailure)
{
    ...
    CallbackStatus status(dispatcher);
    node.getRandomOutsByAmounts({1, 2, 3}, 1, outs, [&status](std::error_code ec)
    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getNewBlocksUninitialized)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.getNewBlocks(std::move(knownBlockIds), newBlocks, startHeight,
                         [&](std::error_code ec)
                         { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getTransactionOutsGlobalIndicesUninitialized)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.getTransactionOutsGlobalIndices(Crypto::Hash(), outsGlobalIndices,
                                            [&](std::error_code ec)
                                            { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getRandomOutsByAmountsUninitialized)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.getRandomOutsByAmounts({1, 2, 3}, 1, outs, [&](std::error_code ec)
    { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, relayTransactionUninitialized)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.relayTransaction(CryptoNote::Transaction(), [&](std::error_code ec)
    { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHeightEmpty)
{
    ...
    CallbackStatus status(dispatcher);
    node.getBlocks(blockHeights, blocks, [&status](std::error_code ec)
    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
...
TEST_F(InProcessNodeTests, getBlocksByHeightMany)
{
    ...
    for (auto iter = generator.getBlockchain().begin() + 1; iter != generator.getBlockchain().end(); iter++)
    {
        ...
        blockHeights.push_back(
                std::move(boost::get<CryptoNote::BaseInput>(iter->baseTransaction.inputs.front()).blockIndex));
        coreStub.addBlock(*iter);
    }

    CallbackStatus status(dispatcher);
    node.getBlocks(blockHeights, actualBlocks, [&status](std::error_code ec)
    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_EQ(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getBlocksByHeightNotInited)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.getBlocks(blockHeights, blocks, [&status](std::error_code ec)
    { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHashEmpty)
{
    ...
    CallbackStatus status(dispatcher);
    node.getBlocks(blockHashes, blocks, [&status](std::error_code ec)
    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_EQ(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getBlocksByHashMany)
{
    ...
    CallbackStatus status(dispatcher);
    node.getBlocks(blockHashes, actualBlocks, [&status](std::error_code ec)
    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_EQ(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getTxEmpty)
{
    ...
    CallbackStatus status(dispatcher);
    node.getTransactions(transactionHashes, transactions,
                         [&status](std::error_code ec)
                         { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_EQ(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getTxMany)
{
    ...
    for (size_t i = 0; i < BLOCKCHAIN_TX_NUMBER; ++i)
    {
        ...
        expectedTransactions.push_back(
                std::make_tuple(tx, CryptoNote::CachedBlock(generator.getBlockchain().back()).getBlockHash(),
                                boost::get<CryptoNote::BaseInput>(
                                        generator.getBlockchain().back().baseTransaction.inputs.front()).blockIndex));
    }
    ...
    for (size_t i = 0; i < POOL_TX_NUMBER; ++i)
    {
        ...
        expectedTransactions.push_back(
                std::make_tuple(tx, boost::value_initialized<Crypto::Hash>(), boost::value_initialized<uint64_t>()));
    }

    CallbackStatus status(dispatcher);
    node.getTransactions(transactionHashes, actualTransactions,
                         [&status](std::error_code ec)
                         { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_EQ(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getTxNotInited)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.getTransactions(transactionHashes, transactions, [&status](std::error_code ec)
    { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, isSynchronized)
{
    ...
    {
        CallbackStatus status(dispatcher);
        node.isSynchronized(syncStatus, [&status](std::error_code ec)
        { status.setStatusRemote(ec); });
        ASSERT_TRUE(status.wait());
        ASSERT_EQ(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, isSynchronizedNotInited)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.isSynchronized(syncStatus, [&status](std::error_code ec)
    { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
}
...
TEST_F(InProcessNodeTests, getLastLocalBlockTimestamp)
{
    ...
    newNode.init([&initStatus](std::error_code ec)
                 { initStatus.setStatusRemote(ec); });
    ASSERT_TRUE(initStatus.wait());

    System::Timer timer(dispatcher);
    timer.sleep(std::chrono::milliseconds(100)); //bad inproc node architecture

    uint64_t timestamp = invokeNodeMethod<uint64_t>([&newNode]()
                                                    { return newNode.getLastLocalBlockTimestamp(); });
...
TEST_F(InProcessNodeTests, getPoolDiffereceNotInited)
{
    ...
    CallbackStatus status(dispatcher);
    newNode.getPoolSymmetricDifference(std::move(knownPoolTxIds), knownBlockId, isBcActual, newTxs, deletedTxIds,
                                       [&status](std::error_code ec)
                                       { status.setStatus(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_NE(std::error_code(), status.getStatus());
...
TEST_F(InProcessNodeTests, getPoolDiffereceActualBC)
{
    ...
    CallbackStatus status(dispatcher);
    node.getPoolSymmetricDifference(std::move(knownPoolTxIds), knownBlockId, isBcActual, newTxs, deletedTxIds,
                                    [&status](std::error_code ec)
                                    { status.setStatusRemote(ec); });
    ASSERT_TRUE(status.wait());
    ASSERT_EQ(std::error_code(), status.getStatus());
