...
void INodeTrivialRefreshStub::getNewBlocks(std::vector<Crypto::Hash> &&knownBlockIds, std::vector<RawBlock> &newBlocks,
                                           uint32_t &startHeight, const Callback &callback)
{
    ...
    std::thread task(
            std::bind(&INodeTrivialRefreshStub::doGetNewBlocks, this, std::move(knownBlockIds), std::ref(newBlocks),
...
void INodeTrivialRefreshStub::doGetNewBlocks(std::vector<Crypto::Hash> knownBlockIds, std::vector<RawBlock> &newBlocks,
                                             uint32_t &startHeight, std::vector<BlockTemplate> blockchain,
                                             const Callback &callback)
{
    ...
    for (const auto &id : knownBlockIds)
    {
        start = std::find_if(blockchain.begin(), blockchain.end(),
                             [&id](BlockTemplate &block)
                             { return CachedBlock(block).getBlockHash() == id; });
...
void INodeTrivialRefreshStub::getTransactionOutsGlobalIndices(const Crypto::Hash &transactionHash,
                                                              ...
                                                              const Callback &callback)
{
    ...
    std::thread task(&INodeTrivialRefreshStub::doGetTransactionOutsGlobalIndices, this, transactionHash,
                     std::ref(outsGlobalIndices), callback);
    task.detach();
}

void INodeTrivialRefreshStub::doGetTransactionOutsGlobalIndices(const Crypto::Hash &transactionHash,
                                                                std::vector<uint32_t> &outsGlobalIndices,
                                                                const Callback &callback)
{
    ContextCounterHolder counterHolder(m_asyncCounter);
...
void INodeTrivialRefreshStub::getRandomOutsByAmounts(std::vector<uint64_t> &&amounts, uint16_t outsCount,
                                                     std::vector<COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> &result,
                                                     const Callback &callback)
{
    m_asyncCounter.addAsyncContext();
    std::thread task(&INodeTrivialRefreshStub::doGetRandomOutsByAmounts, this, amounts, outsCount, std::ref(result),
                     callback);
    task.detach();
}

void INodeTrivialRefreshStub::doGetRandomOutsByAmounts(std::vector<uint64_t> amounts, uint16_t outsCount,
                                                       std::vector<COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS::outs_for_amount> &result,
                                                       const Callback &callback)
{
    ContextCounterHolder counterHolder(m_asyncCounter);
                                          ...
                                          const Callback &callback)
{
    ...
    getNewBlocks(std::move(knownBlockIds), *resultHolder, startHeight,
                 [resultHolder, callback, &startHeight, &newBlocks](std::error_code ec)
                 {
                     if (ec)
                     {
                         callback(ec);
...
void INodeTrivialRefreshStub::getPoolSymmetricDifference(std::vector<Crypto::Hash> &&known_pool_tx_ids,
                                                         ...
                                                         const Callback &callback)
{
    ...
    std::thread task(
            ...
            {
                this->doGetPoolSymmetricDifference(std::move(known_pool_tx_ids), known_block_id, is_bc_actual, new_txs,
                                                   deleted_tx_ids, callback);
            }
    );
    task.detach();
}
                                        ...
                                        std::vector<std::vector<BlockDetails>> &blocks, const Callback &callback)
{
    ...
    std::thread task([=, &blocks]() mutable
                     { doGetBlocks(blockHeights, blocks, callback); });
    task.detach();
}

void INodeTrivialRefreshStub::doGetBlocks(const std::vector<uint32_t> &blockHeights,
                                          std::vector<std::vector<BlockDetails>> &blocks, const Callback &callback)
{
    ContextCounterHolder counterHolder(m_asyncCounter);
    std::unique_lock<std::mutex> lock(m_walletLock);
...
void INodeTrivialRefreshStub::getBlock(const uint32_t blockHeight, BlockDetails &block, const Callback &callback)
{
    ...
    std::thread task([=, &block]() mutable
                     { doGetBlock(blockHeight, block, callback); });
    task.detach();
}
...
void INodeTrivialRefreshStub::getBlocks(const std::vector<Crypto::Hash> &blockHashes, std::vector<BlockDetails> &blocks,
                                        const Callback &callback)
{
    m_asyncCounter.addAsyncContext();

    std::thread{[&blockHashes, &blocks, callback, this]
                { doGetBlocks(blockHashes, blocks, callback); }}.detach();
}

void
INodeTrivialRefreshStub::doGetBlocks(const std::vector<Crypto::Hash> &blockHashes, std::vector<BlockDetails> &blocks,
                                     const Callback &callback)
{
    ContextCounterHolder counterHolder(m_asyncCounter);
    std::unique_lock<std::mutex> lock(m_walletLock);
...
void INodeTrivialRefreshStub::doGetBlocks(uint64_t timestampBegin, uint64_t timestampEnd, uint32_t blocksNumberLimit,
...
void INodeTrivialRefreshStub::doGetTransactionsByPaymentId(const Crypto::Hash &paymentId,
...
void INodeTrivialRefreshStub::getTransactions(const std::vector<Crypto::Hash> &transactionHashes,
                                              std::vector<TransactionDetails> &transactions, const Callback &callback)
{
    m_asyncCounter.addAsyncContext();

    std::thread task([=, &transactions]() mutable
                     { doGetTransactions(transactionHashes, transactions, callback); });
    task.detach();
}

void INodeTrivialRefreshStub::doGetTransactions(const std::vector<Crypto::Hash> &transactionHashes,
