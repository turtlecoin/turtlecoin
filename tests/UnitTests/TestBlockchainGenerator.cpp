...
public:
        ...
        CryptoNote::decompose_amount_into_digits(this->m_source_amount, 0,
                                                 [&](uint64_t chunk)
                                                 {
                                                     destinations.push_back(
                                                             CryptoNote::TransactionDestinationEntry(chunk, address));
                                                 },
                                                 [&](uint64_t a_dust)
                                                 {
                                                     destinations.push_back(
                                                             CryptoNote::TransactionDestinationEntry(a_dust, address));
                                                 });

        CryptoNote::constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources,
                                         destinations, std::vector<uint8_t>(), tx, unlockTime, m_logger);
    }
    ...
    void generateSingleOutputTx(const AccountPublicAddress &address, uint64_t amount, Transaction &tx)
    {
        ...
        constructTransaction(this->m_miners[this->real_source_idx].getAccountKeys(), this->m_sources, destinations,
                             std::vector<uint8_t>(), tx, 0, m_logger);
    }
};
...
bool
TestBlockchainGenerator::getTransactionByHash(const Crypto::Hash &hash, CryptoNote::Transaction &tx, bool checkTxPool)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
bool
TestBlockchainGenerator::generateTransactionsInOneBlock(const CryptoNote::AccountPublicAddress &address, size_t n)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
size_t TestBlockchainGenerator::getGeneratedTransactionsNumber(uint32_t index)
{
    ...
    return std::accumulate(
            ...
            [](size_t sum, const CryptoNote::BlockTemplate &block)
            {
                return sum + block.transactionHashes.size() + 1;
            });
}

bool
TestBlockchainGenerator::getSingleOutputTransaction(const CryptoNote::AccountPublicAddress &address, uint64_t amount)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
void TestBlockchainGenerator::addToBlockchain(const std::vector<CryptoNote::Transaction> &txs,
                                              const CryptoNote::AccountBase &minerAddress)
{
...
void TestBlockchainGenerator::getPoolSymmetricDifference(std::vector<Crypto::Hash> &&known_pool_tx_ids,
                                                         Crypto::Hash known_block_id, bool &is_bc_actual,
                                                         std::vector<CryptoNote::Transaction> &new_txs,
...
bool TestBlockchainGenerator::getTransactionGlobalIndexesByHash(const Crypto::Hash &transactionHash,
