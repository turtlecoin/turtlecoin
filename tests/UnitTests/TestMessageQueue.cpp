...
{
    for (IntrusiveLinkedList<MessageQueue<BlockchainMessage>>::iterator iter = blockchainMessageQueueList.begin();
         iter != blockchainMessageQueueList.end(); ++iter)
    {
        iter->push(message);
    }
}
...
TEST_F(MessageQueueTest, manyMessagesManyListeners)
{
    ...
    for (size_t i = 0; i < NUMBER_OF_LISTENERS; ++i)
    {
        ...
        quards[i] = std::unique_ptr<MesageQueueGuard<MessageQueueTest, BlockchainMessage>>(
                new MesageQueueGuard<MessageQueueTest, BlockchainMessage>(*this, *queues[i]));
    }
