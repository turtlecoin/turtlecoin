...
struct IWalletBaseStub : public CryptoNote::IWallet, public CryptoNote::IFusionManager
{
    ...
    virtual void
    exportWallet(const std::string &path, bool encrypt = true, WalletSaveLevel saveLevel = WalletSaveLevel::SAVE_ALL,
    ...
    virtual std::vector<std::string> createAddressList(
            const std::vector<Crypto::SecretKey> &spendSecretKeys) override
    ...
    getTransactionTransfer(size_t transactionIndex, size_t transferIndex) const override
    { return WalletTransfer(); }

    virtual WalletTransactionWithTransfers
    getTransaction(const Crypto::Hash &transactionHash) const override
    { return WalletTransactionWithTransfers(); }

    virtual std::vector<TransactionsInBlockInfo>
    getTransactions(const Crypto::Hash &blockHash, size_t count) const override
    { return {}; }

    virtual std::vector<TransactionsInBlockInfo>
    getTransactions(uint32_t blockIndex, size_t count) const override
    { return {}; }
...
protected:
    ...
    std::unique_ptr<WalletService>
    createWalletService(CryptoNote::IWallet &wallet, CryptoNote::IFusionManager &fusionManager);
...
std::unique_ptr<WalletService>
WalletServiceTest::createWalletService(CryptoNote::IWallet &wallet, CryptoNote::IFusionManager &fusionManager)
{
    return std::unique_ptr<WalletService>(
            new WalletService(currency, dispatcher, nodeStub, wallet, fusionManager, walletConfig, logger));
}
...
{
    WalletGetBlockHashesStub wallet(dispatcher);
    std::generate_n(std::back_inserter(wallet.blockHashes), 10, [this]()
...
void WalletServiceTest_getTransactions::SetUp()
{
    ...
    block.transactions.push_back(
            WalletTransactionWithTransfersBuilder().addTransfer(RANDOM_ADDRESS1, 222).addTransfer(RANDOM_ADDRESS2,
                                                                                                  33333).transaction(
...
TEST_F(WalletServiceTest_getTransactions, addressesFilter_emptyReturnsTransaction)
{
    ...
    ASSERT_EQ(Common::podToHex(testTransactions[0].transactions[0].transaction.hash),
              transactions[0].transactions[0].transactionHash);
}
...
TEST_F(WalletServiceTest_getTransactions, paymentIdFilter_nonExistentReturnsNoTransaction)
{
    ...
    std::vector<TransactionsInBlockRpcInfo> transactions;
    auto ec = service->getTransactions({}, 0, 1, "dfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdfdf",
                                       transactions);
...
{
    WalletTransferStub(System::Dispatcher &dispatcher, const Crypto::Hash &hash) : IWalletBaseStub(dispatcher),
                                                                                   hash(hash)
    {
    }
...
bool isEquivalent(const SendTransaction::Request &request, const TransactionParameters &params)
{
    ...
    return std::make_tuple(request.sourceAddresses, orders, request.fee, request.anonymity, extra, request.unlockTime)
           ==
           std::make_tuple(params.sourceAddresses, params.destinations, params.fee, params.mixIn,
                           Common::toHex(Common::asBinaryArray(params.extra)), params.unlockTimestamp);
}
...
{
    WalletMakeTransactionStub(System::Dispatcher &dispatcher, const Crypto::Hash &hash) : IWalletBaseStub(dispatcher),
                                                                                          hash(hash)
    {
    }
...
{
    WalletGetDelayedTransactionIdsStub(System::Dispatcher &dispatcher, const Crypto::Hash &hash) : IWalletBaseStub(
            dispatcher), hash(hash)
    {
    }
...
TEST_F(WalletServiceTest_sendFusionTransaction, failsOnWrongSourceAddress)
{
    ...
    auto ec = service->sendFusionTransaction(TEST_THRESHOLD, TEST_MIXIN, {testAddress1, "WRONG ADDRESS"}, testAddress2,
                                             transactionHash);
    ASSERT_EQ(make_error_code(CryptoNote::error::BAD_ADDRESS), ec);
}
...
TEST_F(WalletServiceTest_sendFusionTransaction, failsOnWrongDestinationAddress)
{
    ...
    std::string transactionHash;
    auto ec = service->sendFusionTransaction(TEST_THRESHOLD, TEST_MIXIN, {testAddress1}, "WRONG ADDRESS",
                                             transactionHash);
    ASSERT_EQ(make_error_code(CryptoNote::error::BAD_ADDRESS), ec);
}
...
TEST_F(WalletServiceTest_sendFusionTransaction, acceptsEmptySourceAddresses)
{
    ...
    std::string transactionHash;
    ASSERT_FALSE(static_cast<bool>(service->sendFusionTransaction(TEST_THRESHOLD, TEST_MIXIN, {}, testAddress2,
                                                                  transactionHash)));
}
...
TEST_F(WalletServiceTest_sendFusionTransaction, acceptsEmptyDestinationAddress)
{
    ...
    std::string transactionHash;
    ASSERT_FALSE(static_cast<bool>(service->sendFusionTransaction(TEST_THRESHOLD, TEST_MIXIN, {testAddress1}, "",
                                                                  transactionHash)));
}
...
TEST_F(WalletServiceTest_sendFusionTransaction, correctlyPassAgrumentsToWallet)
{
    ...
    ASSERT_FALSE(
            static_cast<bool>(service->sendFusionTransaction(TEST_THRESHOLD, TEST_MIXIN, sourceAddresses, testAddress2,
                                                             transactionHash)));
...
TEST_F(WalletServiceTest_sendFusionTransaction, returnsCorrectFustionTransactoinHash)
{
    ...
    std::string transactionHash;
    ASSERT_FALSE(
            static_cast<bool>(service->sendFusionTransaction(TEST_THRESHOLD, TEST_MIXIN, {testAddress1}, testAddress2,
                                                             transactionHash)));
...
TEST_F(WalletServiceTest_estimateFusion, failsOnWrongSourceAddress)
{
    ...
    auto ec = service->estimateFusion(TEST_THRESHOLD, {testAddress1, "WRONG ADDRESS"}, fusionReadyCount,
                                      totalOutputCount);
    ASSERT_EQ(make_error_code(CryptoNote::error::BAD_ADDRESS), ec);
}
...
TEST_F(WalletServiceTest_estimateFusion, correctlyPassAgrumentsToWallet)
{
    ...
    uint32_t totalOutputCount;
    std::vector<std::string> sourceAddresses = {testAddress1, testAddress2};
    ASSERT_FALSE(static_cast<bool>(service->estimateFusion(TEST_THRESHOLD, sourceAddresses, fusionReadyCount,
                                                           totalOutputCount)));
...
TEST_F(WalletServiceTest_estimateFusion, returnsDataReceivedFromWallet)
{
    ...
    uint32_t fusionReadyCount;
    uint32_t totalOutputCount;
    ASSERT_FALSE(static_cast<bool>(service->estimateFusion(TEST_THRESHOLD, {testAddress1}, fusionReadyCount,
                                                           totalOutputCount)));
