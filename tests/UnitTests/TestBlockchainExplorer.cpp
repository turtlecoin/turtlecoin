...
namespace
{
    ...
    struct CallbackStatus
    {
        ...
        bool wait()
        { return waiter.wait_for(std::chrono::milliseconds(3000)); }

        bool ok()
        { return waiter.wait_for(std::chrono::milliseconds(3000)) && !static_cast<bool>(code); }
    ...
    public:
        ...
        virtual void blockchainUpdated(const std::vector<BlockDetails> &newBlocks,
                                       const std::vector<BlockDetails> &orphanedBlocks) override
        {
        ...
        virtual void poolUpdated(const std::vector<TransactionDetails> &newTransactions,
                                 const std::vector<std::pair<Hash, TransactionRemoveReason>> &removedTransactions) override
        {
        ...
        void setCallback(const std::function<void(const std::vector<BlockDetails> &newBlocks,
                                                  const std::vector<BlockDetails> &orphanedBlocks)> &cb)
        {
            blockchainUpdatedCallback = cb;
        }

        void setCallback(const std::function<void(const std::vector<TransactionDetails> &newTransactions,
    ...
    private:
        std::function<void(const std::vector<BlockDetails> &newBlocks,
                           const std::vector<BlockDetails> &orphanedBlocks)> blockchainUpdatedCallback;
        std::function<void(const std::vector<TransactionDetails> &newTransactions,
                           const std::vector<std::pair<Hash, TransactionRemoveReason>> &removedTransactions)> poolUpdatedCallback;
...
TEST_F(BlockchainExplorerTests, getPoolStateEmpty)
{
    ...
    ASSERT_TRUE(blockchainExplorer.getPoolState(knownPoolTransactionHashes, knownBlockchainTop, isBlockchainActual,
                                                newTransactions, removedTransactions));
...
TEST_F(BlockchainExplorerTests, getPoolStateNotInited)
{
    ...
    ASSERT_ANY_THROW(newExplorer.getPoolState(knownPoolTransactionHashes, knownBlockchainTop, isBlockchainActual,
                                              newTransactions, removedTransactions));
}
...
{
    ASSERT_EQ(blockchainExplorer.getFullRewardMaxBlockSize(1),
              parameters::CRYPTONOTE_BLOCK_GRANTED_FULL_REWARD_ZONE_V1);
    ASSERT_EQ(blockchainExplorer.getFullRewardMaxBlockSize(2),
...
TEST_F(BlockchainExplorerTests, blockchainUpdatedEmpty)
{
    ...
    observer.setCallback(
            static_cast<std::function<void(const std::vector<BlockDetails> &newBlocks,
                            ...
                            const std::vector<std::pair<Hash, TransactionRemoveReason>> &removedTransactions)
                        {
                            ...
                            for (const Hash &poolTxHash : poolTxs)
                            {
                                auto iter = std::find_if(removedTransactions.begin(), removedTransactions.end(),
                                                         [&poolTxHash](
                                                                 const std::pair<Hash, TransactionRemoveReason> &txDetails) -> bool
                                                         {
...
TEST_F(BlockchainExplorerTests, poolUpdatedManyNotSynchronized)
{
    ...
    std::function<void(const std::vector<BlockDetails> &newBlocks,
                       const std::vector<BlockDetails> &orphanedBlocks)> cb1 =
            [&status, this](const std::vector<BlockDetails> &newBlocks,
                            const std::vector<BlockDetails> &orphanedBlocks)
            {};
    observer.setCallback(cb1);
...
{
public:
    GetBlocksByTimestampsNode(TestBlockchainGenerator &generator, bool consumerTests = false) : INodeTrivialRefreshStub(
            generator, consumerTests)
    {}
...
{
public:
    GetTransactionHashByPaymentIdNode(TestBlockchainGenerator &generator, bool consumerTests = false)
            : INodeTrivialRefreshStub(generator, consumerTests)
    {
    }
...
TEST_F(BlockchainExplorerTests, getTransactionsByPaymentIdFail)
{
    ...
    Hash randomPaymentId;
    std::generate(std::begin(randomPaymentId.data), std::end(randomPaymentId.data), []
    { return rand(); });
