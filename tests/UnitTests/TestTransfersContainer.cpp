...
namespace
{
    ...
    protected:
        ...
        addSpendingTransaction(const Hash &sourceTx, uint32_t height, uint32_t outputIndex,
                               uint64_t amount = TEST_OUTPUT_AMOUNT)
        {
            ...
            if (inputAmount > amount)
            {
                transfers.emplace_back(
                        builder.addTestKeyOutput(inputAmount - amount, outputIndex + 1, account)); // change
            }
...
TEST_F(TransfersContainer_addTransaction, spendingUnconfirmedOutputFails)
{
    ...
    ASSERT_ANY_THROW(
            container.addTransaction(blockInfo(WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT), *spendingTx.build(), {}));
}
...
TEST_F(TransfersContainer_addTransaction, spendingConfirmedOutputWithConfirmedTxSucceed)
{
    ...
    auto spendingTx = addSpendingTransaction(tx->getTransactionHash(),
                                             TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE,
                                             TEST_TRANSACTION_OUTPUT_GLOBAL_INDEX + 1);
...
{
    ASSERT_ANY_THROW(
            markConfirmed(addTransaction()->getTransactionHash(), WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT));
}
...
TEST_F(TransfersContainer_detach, detachConfirmedSpendingTransaction)
{
    ...
    auto spendingTx = addSpendingTransaction(
            tx->getTransactionHash(), TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE,
...
TEST_F(TransfersContainer_detach, threeRelatedTransactions)
{
    ...
    auto spendingTx2 = addSpendingTransaction(
            spendingTx1->getTransactionHash(), TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE * 2,
            TEST_TRANSACTION_OUTPUT_GLOBAL_INDEX + 2, TEST_OUTPUT_AMOUNT / 2);
...
TEST_F(TransfersContainer_detach, confirmedWithUnconfirmedSpendingTransaction_H1)
{
    ...
    auto spendingTx = addSpendingTransaction(
            tx->getTransactionHash(), WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT,
            UNCONFIRMED_TRANSACTION_GLOBAL_OUTPUT_INDEX);
...
TEST_F(TransfersContainer_balance, treatsUnconfirmedTransfersAsLocked)
{
    ...
    ASSERT_EQ(AMOUNT_1,
              container.balance(ITransfersContainer::IncludeStateLocked | ITransfersContainer::IncludeTypeAll));
}
...
TEST_F(TransfersContainer_balance, handlesTransferStateUnLocked)
{
    ...
    ASSERT_EQ(AMOUNT_2,
              container.balance(ITransfersContainer::IncludeStateUnlocked | ITransfersContainer::IncludeTypeAll));
}
