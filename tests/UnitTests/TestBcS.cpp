...
public:
    ...
    INodeNonTrivialRefreshStub(TestBlockchainGenerator &generator) : INodeTrivialRefreshStub(generator),
                                                                     blocksWasQueried(false), poolWasQueried(false)
    {}

    virtual void
    queryBlocks(std::vector<Hash> &&knownBlockIds, uint64_t timestamp, std::vector<BlockShortEntry> &newBlocks,
    ...
    virtual void
    getPoolSymmetricDifference(std::vector<Hash> &&known_pool_tx_ids, Hash known_block_id, bool &is_bc_actual,
                               std::vector<std::unique_ptr<ITransactionReader>> &new_txs,
                               std::vector<Hash> &deleted_tx_ids, const Callback &callback) override
    {
        poolWasQueried = true;
        INodeTrivialRefreshStub::getPoolSymmetricDifference(std::move(known_pool_tx_ids), known_block_id, is_bc_actual,
                                                            new_txs, deleted_tx_ids, callback);
    }
...
public:
    ...
    INodeFunctorialStub(TestBlockchainGenerator &generator)
            : INodeNonTrivialRefreshStub(generator), queryBlocksFunctor(
            [](const std::vector<Hash> &, uint64_t, std::vector<BlockShortEntry> &, uint32_t &,
               const Callback &) -> bool
            { return true; }), getPoolSymmetricDifferenceFunctor(
            [](const std::vector<Hash> &, Hash, bool &, std::vector<std::unique_ptr<ITransactionReader>> &,
               std::vector<Hash> &, const Callback &) -> bool
            { return true; })
    {
    }
                ...
                uint32_t &startHeight, const Callback &callback) override
    {
        ...
        {
            INodeNonTrivialRefreshStub::queryBlocks(std::move(knownBlockIds), timestamp, newBlocks, startHeight,
                                                    callback);
        }
    }
    ...
    getPoolSymmetricDifference(std::vector<Hash> &&known_pool_tx_ids, Hash known_block_id, bool &is_bc_actual,
                               ...
                               std::vector<Hash> &deleted_tx_ids, const Callback &callback) override
    {
        if (getPoolSymmetricDifferenceFunctor(known_pool_tx_ids, known_block_id, is_bc_actual, new_txs, deleted_tx_ids,
                                              callback))
        {
            INodeNonTrivialRefreshStub::getPoolSymmetricDifference(std::move(known_pool_tx_ids), known_block_id,
                                                                   is_bc_actual, new_txs, deleted_tx_ids, callback);
        }
    }

    std::function<bool(const std::vector<Hash> &, uint64_t, std::vector<BlockShortEntry> &, uint32_t &,
                       const Callback &)> queryBlocksFunctor;
    std::function<bool(const std::vector<Hash> &, Hash, bool &, std::vector<std::unique_ptr<ITransactionReader>> &,
                       std::vector<Hash> &, const Callback &)> getPoolSymmetricDifferenceFunctor;
};
...
{
public:
    IBlockchainSynchronizerFunctorialObserver() : updFunc([](uint32_t, uint32_t)
...
public:
    ...
    virtual std::error_code onPoolUpdated(const std::vector<std::unique_ptr<ITransactionReader>> &addedTransactions,
                                          const std::vector<Hash> &deletedTransactions) override
    {
...
public:
    ...
    void checkSyncedBlockchains()
    {
        ...
        std::transform(
                ...
                { return CachedBlock(b).getBlockHash(); });
...
public:
    ...
    std::function<std::error_code(const std::vector<std::unique_ptr<ITransactionReader>> &,
                                  const std::vector<Hash> &)> onPoolUpdatedFunctor;
};
...
TEST_F(BcSTest, firstPoolSynchronizationCheck)
{
    ...
    c1.onPoolUpdatedFunctor = [&](const std::vector<std::unique_ptr<ITransactionReader>> &new_txs,
                                  const std::vector<Hash> &deleted) -> std::error_code
    {
    ...
    c2.onPoolUpdatedFunctor = [&](const std::vector<std::unique_ptr<ITransactionReader>> &new_txs,
                                  const std::vector<Hash> &deleted) -> std::error_code
    {
    ...
    m_node.getPoolSymmetricDifferenceFunctor = [&](const std::vector<Hash> &known, Hash last, bool &is_actual,
                                                   std::vector<std::unique_ptr<ITransactionReader>> &new_txs,
                                                   std::vector<Hash> &deleted, const INode::Callback &callback)
    {
        is_actual = true;
        requestsCount++;
...
public:
    ...
    FunctorialBlockhainConsumerStub(const Hash &genesisBlockHash) : ConsumerStub(genesisBlockHash),
                                                                    onBlockchainDetachFunctor([](uint32_t)
                                                                                              {})
    {}
...
TEST_F(BcSTest, checkINodeError)
{
    ...
    m_node.queryBlocksFunctor = [](const std::vector<Hash> &knownBlockIds, uint64_t timestamp,
...
TEST_F(BcSTest, checkStatePreservingBetweenSynchronizations)
{
    ...
    m_node.queryBlocksFunctor = [&receivedLastBlockHash](const std::vector<Hash> &knownBlockIds, uint64_t timestamp,
                                                         std::vector<BlockShortEntry> &newBlocks, uint32_t &startHeight,
                                                         const INode::Callback &callback) -> bool
    {
...
TEST_F(BcSTest, checkTxOrder)
{
    ...
    m_node.queryBlocksFunctor = [&bse, &requestNumber](const std::vector<Hash> &knownBlockIds, uint64_t timestamp,
                                                       std::vector<BlockShortEntry> &newBlocks, uint32_t &startHeight,
                                                       const INode::Callback &callback) -> bool
    {
        startHeight = 1;
