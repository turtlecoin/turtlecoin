...
namespace
{
    ...
    protected:
        ...
        void createTestFile(const std::string &path, uint64_t capacity, Iterator first, Iterator last,
        ...
        {
            createTestFile(path, TEST_VECTOR_CAPACITY, TEST_VECTOR_DATA.begin(), TEST_VECTOR_DATA.end(),
                           TEST_FILE_PREFIX, TEST_FILE_SUFFIX);
        }

        static void readVectorFile(const std::string &path, uint64_t prefixSize, uint64_t *capacity = nullptr,
                                   uint64_t *size = nullptr,
                                   std::vector<char> *data = nullptr, std::vector<char> *prefix = nullptr,
                                   std::vector<char> *suffix = nullptr)
        {
        ...
        static void readVectorFile(const std::string &path, uint64_t *capacity = nullptr, uint64_t *size = nullptr,
    ...
    TEST_F(FileMappedVectorTest, resizePrefixCorrectlyShrinksPrefix)
    {
        ...
        ASSERT_EQ(TEST_FILE_PREFIX.substr(0, TEST_FILE_PREFIX.size() - 1),
                  std::string(vec.prefix(), vec.prefix() + vec.prefixSize()));
    ...
    TEST_F(FileMappedVectorTest, resizeSuffixCorrectlyShrinksSuffix)
    {
        ...
        ASSERT_EQ(TEST_FILE_SUFFIX.substr(0, TEST_FILE_SUFFIX.size() - 1),
                  std::string(vec.suffix(), vec.suffix() + vec.suffixSize()));
