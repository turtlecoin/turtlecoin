...
namespace
{
    ...
    public:
        DatabaseBlockchainCacheTests()
                : currency(CurrencyBuilder(logger).currency()), blockchainCacheFactory("", logger),
                  blockchain(currency, database, blockchainCacheFactory, logger), generator(currency)
        {
        }
        ...
        void SetUp() override
        {
            ...
            for (auto &block : generator.getBlockchain())
            {
                ...
                blockchain.pushBlock(cached, {}, state, 0, 0, 0,
                                     {toBinaryArray(block), {}}); // TODO: add coins, block sizes, etc
            }
...
{
    ASSERT_EQ(generator.getBlockchain().size(),
              DatabaseBlockchainCache(currency, database, blockchainCacheFactory, logger).getTopBlockIndex());
}
...
TEST_F(DatabaseBlockchainCacheTests, RawBlocksWithTxsSerialization)
{
    ...
    ASSERT_NO_THROW(std::transform(std::begin(lastBlock.transactionHashes), std::end(lastBlock.transactionHashes),
