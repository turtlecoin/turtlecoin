...
namespace
{
    ...
    TransactionOutputInformationIn addTestKeyOutput(ITransaction &transaction, uint64_t amount,
                                                    ...
                                                    const AccountKeys &senderKeys = generateAccountKeys())
    {
...
namespace CryptoNote
{
    ...
    public:
        ...
        size_t addTestInput(uint64_t amount, std::vector<uint32_t> gouts,
                            const AccountKeys &senderKeys = generateAccountKeys());

        void addInput(const AccountKeys &senderKeys, const TransactionOutputInformation &t);

        // outputs
        TransactionOutputInformationIn addTestKeyOutput(uint64_t amount, uint32_t globalOutputIndex,
                                                        const AccountKeys &senderKeys = generateAccountKeys());
    ...
    private:
        ...
        void derivePublicKey(const AccountKeys &reciever, const Crypto::PublicKey &srcTxKey, size_t outputIndex,
                             PublicKey &ephemeralKey)
        {
            Crypto::KeyDerivation derivation;
            Crypto::generate_key_derivation(srcTxKey,
                                            reinterpret_cast<const Crypto::SecretKey &>(reciever.viewSecretKey),
                                            derivation);
