...
protected:
    ...
    addSubscription(const AccountKeys &acc, uint64_t height = 0, uint64_t timestamp = 0, size_t age = 0)
    {
    ...
    ITransfersSubscription &
    addSubscription(TransfersConsumer &consumer, uint64_t height = 0, uint64_t timestamp = 0, size_t age = 0)
    {
...
bool amountFound(const std::vector<TransactionOutputInformation> &outs, uint64_t amount)
{
    return std::find_if(outs.begin(), outs.end(),
                        [amount](const TransactionOutputInformation &inf)
                        { return inf.amount == amount; }) !=
           outs.end();
}
...
TEST_F(TransfersConsumerTest, removeSubscription_OneAddressLeft)
{
    ...
    AccountKeys accountKeys = getAccountKeysWithViewKey(m_accountKeys.address.viewPublicKey,
                                                        m_accountKeys.viewSecretKey);
...
TEST_F(TransfersConsumerTest, onNewBlocks_updateHeight)
{
    ...
    ASSERT_EQ(subscription.transactionSpendableAge,
              m_consumer.onNewBlocks(blocks.get(), static_cast<uint32_t>(subscription.syncStart.height + 1),
                                     static_cast<uint32_t>(subscription.transactionSpendableAge)));
...
TEST_F(TransfersConsumerTest, onNewBlocks_markTransactionConfirmed)
{
    ...
    auto softLockedOuts = container.getTransactionOutputs(tx->getTransactionHash(),
                                                          ITransfersContainer::IncludeKeyUnlocked);
...
public:
    ...
    {
        std::cout << "Generating " << blocksCount << " blocks, " << blocksCount * txPerBlock << " transactions"
                  << std::endl;
