...
ICoreStub::findBlockchainSupplement(const std::vector<Crypto::Hash> &remoteBlockIds, size_t maxCount,
...
void
ICoreStub::set_random_outs(const CryptoNote::COMMAND_RPC_GET_RANDOM_OUTPUTS_FOR_AMOUNTS_response &resp, bool result)
{
    randomOuts = resp;
...
bool ICoreStub::getPoolChanges(const Crypto::Hash &tailBlockId, const std::vector<Crypto::Hash> &knownTxsIds,
                               std::vector<CryptoNote::BinaryArray> &addedTxs,
                               std::vector<Crypto::Hash> &deletedTxsIds) const
{
...
bool ICoreStub::getPoolChangesLite(const Crypto::Hash &tailBlockId, const std::vector<Crypto::Hash> &knownTxsIds,
                                   std::vector<CryptoNote::TransactionPrefixInfo> &addedTxs,
                                   std::vector<Crypto::Hash> &deletedTxsIds) const
{
...
bool ICoreStub::queryBlocks(const std::vector<Crypto::Hash> &block_ids, uint64_t timestamp,
                            ...
                            std::vector<CryptoNote::BlockFullInfo> &entries) const
{
    //stub
    return true;
}
...
bool ICoreStub::queryBlocksLite(const std::vector<Crypto::Hash> &block_ids, uint64_t timestamp,
                                uint32_t &start_height, uint32_t &current_height, uint32_t &full_offset,
                                std::vector<CryptoNote::BlockShortInfo> &entries) const
{
    //stub
    return true;
}
...
void ICoreStub::getBlocks(const std::vector<Crypto::Hash> &blockHashes, std::vector<CryptoNote::RawBlock> &blocks,
...
bool ICoreStub::getTransactionGlobalIndexes(const Crypto::Hash &transactionHash,
...
bool ICoreStub::getRandomOutputs(uint64_t amount, uint16_t count, std::vector<uint32_t> &globalIndexes,
...
bool ICoreStub::getBlockTemplate(CryptoNote::BlockTemplate &b, const CryptoNote::AccountPublicAddress &adr,
                                 const CryptoNote::BinaryArray &extraNonce, CryptoNote::Difficulty &difficulty,
...
{
    return transactions.find(transactionHash) != transactions.end() ||
           transactionPool.find(transactionHash) != transactionPool.end();
}
