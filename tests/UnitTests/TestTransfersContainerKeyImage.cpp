...
namespace
{
    ...
    protected:
        ...
        addTransactionWithFixedKey(uint32_t height, size_t inputs = 1, uint64_t amount = TEST_OUTPUT_AMOUNT,
                                   uint32_t txIndex = 0)
        {
        ...
        std::unique_ptr<ITransactionReader>
        addTransaction(uint32_t height = WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT)
        {
        ...
        std::unique_ptr<ITransactionReader>
        addSpendingTransaction(const Hash &sourceTx, uint32_t height, uint32_t outputIndex,
                               uint64_t amount = TEST_OUTPUT_AMOUNT, bool fixedKey = false)
        {
...
TEST_F(TransfersContainerKeyImage, addTransaction_addingSecondUnconfirmedTransferHidesBothUnconfirmedTransfers)
{
    ...
    ASSERT_EQ(0, container.balance(
            ITransfersContainer::IncludeAllLocked)); // transactions' outputs should shadow one another
...
TEST_F(TransfersContainerKeyImage, addTransaction_unconfirmedTransferAddedAfterConfirmedBecomeHidden)
{
    ...
    ASSERT_TRUE(
            container.addTransaction({WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT, 100000}, *tx2.build(), {tx2out}));
...
TEST_F(TransfersContainerKeyImage, addTransaction_secondConfirmedTransferAddedAsHidden_TransfersInDifferentBlocks)
{
    ...
    ASSERT_EQ(TEST_OUTPUT_AMOUNT,
              container.balance(ITransfersContainer::IncludeStateSoftLocked | ITransfersContainer::IncludeTypeAll));
...
TEST_F(TransfersContainerKeyImage, addTransaction_confirmedTransferAddedAfterSpentBecomeHidden)
{
    auto tx1 = addTransactionWithFixedKey(TEST_BLOCK_HEIGHT);
    container.advanceHeight(TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE);
    addSpendingTransaction(tx1->getTransactionHash(), TEST_BLOCK_HEIGHT + 1, TEST_TRANSACTION_OUTPUT_GLOBAL_INDEX,
                           TEST_OUTPUT_AMOUNT);
...
TEST_F(TransfersContainerKeyImage,
       markTransactionConfirmed_confirmingOneOfAFewUnconfirmedTransfersMakesThisTransferVisible)
{
    ...
    ASSERT_EQ(TEST_OUTPUT_AMOUNT * 2,
              container.balance(ITransfersContainer::IncludeStateSoftLocked | ITransfersContainer::IncludeTypeAll));
...
TEST_F(TransfersContainerKeyImage, markTransactionConfirmed_oneSpentOtherUnconfirmed_confirmingOneUnconfirmed)
{
    ...
    ASSERT_TRUE(container.markTransactionConfirmed(
            {TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE + 1, TEST_TIMESTAMP, 0}, tx2->getTransactionHash(),
...
public:
    ...
    bool spendOutput(const TransactionOutputInformation &outInfo)
    {
        ...
        return container.addTransaction({TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE, TEST_TIMESTAMP, 0},
                                        *spendTx.build(), {});
    }
};
...
TEST_F(TransfersContainerKeyImage_Spend,
       spendingUnconfirmedTransferIfThereIsConfirmedWithAnotherAmountCausesException)
{
    auto tx1 = addTransactionWithFixedKey(TEST_BLOCK_HEIGHT);
    auto tx2 = addTransactionWithFixedKey(WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT, 2, TEST_OUTPUT_AMOUNT * 2);
...
TEST_F(TransfersContainerKeyImage_Remove, removeUnconfirmedTxSpendingVisibleOutput)
{
    ...
    auto spendingTx = addSpendingTransaction(tx1->getTransactionHash(),
                                             WALLET_LEGACY_UNCONFIRMED_TRANSACTION_HEIGHT, UNCONFIRMED,
                                             TEST_OUTPUT_AMOUNT);
...
TEST_F(TransfersContainerKeyImage, removingOneOfTwoUnconfirmedTransfersIfThereIsSpentTransferDoesNotAffectBalance)
{
    ...
    container.advanceHeight(TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE);
    auto spendingTx = addSpendingTransaction(tx1->getTransactionHash(),
                                             TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE + 1,
                                             TEST_TRANSACTION_OUTPUT_GLOBAL_INDEX, TEST_OUTPUT_AMOUNT);
    ASSERT_EQ(0, container.balance(ITransfersContainer::IncludeAll));
...
TEST_F(TransfersContainerKeyImage, removeConfirmed_oneSpentOneConfirmed)
{
    ...
    auto tx2 = addTransactionWithFixedKey(TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE + 1, 2,
                                          TEST_OUTPUT_AMOUNT * 2);
    ASSERT_EQ(0, container.balance(ITransfersContainer::IncludeAll));
...
TEST_F(TransfersContainerKeyImage, removeConfirmed_oneSpentTwoConfirmed)
{
    ...
    auto tx2 = addTransactionWithFixedKey(TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE + 1, 2,
                                          TEST_OUTPUT_AMOUNT * 2);
    auto tx3 = addTransactionWithFixedKey(TEST_BLOCK_HEIGHT + TEST_TRANSACTION_SPENDABLE_AGE + 2, 3,
                                          TEST_OUTPUT_AMOUNT * 3);
