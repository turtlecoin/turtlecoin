...
TEST_F(BlockchainCacheTests, pushBlockMany)
{
    ...
    for (size_t i = 1; i < bcCopy.size(); ++i)
    { //Skip genesis block
        ASSERT_NO_FATAL_FAILURE(
                blockCache.pushBlock(CachedBlock(bcCopy.at(i)), transactions, validatorState, SIZE, REWARD, DIFFICULTY,
                                     RawBlock()));
    }
...
TEST_F(BlockchainCacheTests, getTopBlockIndexChain)
{
    ...
    ASSERT_NO_FATAL_FAILURE(
            otherCache.pushBlock(nextBlock, transactions, validatorState, SIZE, REWARD, DIFFICULTY, RawBlock()));
...
TEST_F(BlockchainCacheTests, getTopBlockHash)
{
    ...
    TransactionValidatorState validatorState;
    ASSERT_NO_FATAL_FAILURE(
            blockCache.pushBlock(block, transactions, validatorState, SIZE, REWARD, DIFFICULTY, RawBlock()));
...
TEST_F(BlockchainCacheTests, getBlockHashesChained)
{
    ...
    for (size_t i = start; i < generator.getBlockchain().size(); ++i)
    {
        ...
        ASSERT_NO_FATAL_FAILURE(
                otherCache.pushBlock(block, transactions, validatorState, SIZE, REWARD, DIFFICULTY, RawBlock()));
    }
