...
namespace
{
    ...
    TEST_F(NodeTxPoolSyncTest,
           TxPoolsAreRequestedRightAfterANodeIsConnectedToAnotherIfTheirBlockchainsAreSynchronized)
    {
        ...
        ASSERT_TRUE(mineBlocks(*nodeDaemons[NODE_0], minerAccount.getAccountKeys().address,
        ...
        Hash txHash1;
        ASSERT_FALSE(static_cast<bool>(wallet1.sendTransaction(m_currency.accountAddressAsString(minerAccount),
                                                               m_currency.coin(), txHash1)));
        ...
        Hash txHash2;
        ASSERT_FALSE(static_cast<bool>(wallet2.sendTransaction(m_currency.accountAddressAsString(minerAccount),
                                                               m_currency.coin(), txHash2)));
    ...
    TEST_F(NodeTxPoolSyncTest, TxPoolsAreRequestedRightAfterInitialBlockchainsSynchronization)
    {
        ...
        ASSERT_TRUE(mineBlocks(*nodeDaemons[NODE_3], minerAccount.getAccountKeys().address,
                               m_currency.minedMoneyUnlockWindow()));
        wallet2.waitForSynchronizationToHeight(m_currency.minedMoneyUnlockWindow() + 3);
    ...
    TEST_F(NodeTxPoolSyncTest, TxPoolsAreRequestedRightAfterSwitchingToAlternativeChain)
    {
        ...
        Hash txHash0;
        ASSERT_FALSE(
                static_cast<bool>(wallet0.sendTransaction(wallet0.wallet()->getAddress(), m_currency.coin(), txHash0)));

        // This transaction is valid only in alternative chain 1
        Hash txHash1;
        ASSERT_FALSE(
                static_cast<bool>(wallet1.sendTransaction(wallet0.wallet()->getAddress(), m_currency.coin(), txHash1)));
        ...
        Hash txHash2;
        ASSERT_FALSE(
                static_cast<bool>(wallet2.sendTransaction(wallet0.wallet()->getAddress(), m_currency.coin(), txHash2)));
