...
public:
    ...
    {
        BlockInfo()
                : previousBlockHash(), alreadyGeneratedCoins(0), blockSize(0)
        {
        }

        BlockInfo(Crypto::Hash aPrevId, uint64_t anAlreadyGeneratedCoins, size_t aBlockSize)
                : previousBlockHash(aPrevId), alreadyGeneratedCoins(anAlreadyGeneratedCoins), blockSize(aBlockSize)
        {
        }
    ...
    bool constructBlock(CryptoNote::BlockTemplate &blk, const CryptoNote::BlockTemplate &blkPrev,
    ...
    bool constructBlockManually(CryptoNote::BlockTemplate &blk, const CryptoNote::BlockTemplate &prevBlock,
                                ...
                                const CryptoNote::Difficulty &diffic = 1,
                                const CryptoNote::Transaction &baseTransaction = CryptoNote::Transaction(),
...
inline CryptoNote::Difficulty getTestDifficulty()
{ return 1; }
...
bool constructMinerTxManually(const CryptoNote::Currency &currency, uint8_t blockMajorVersion, uint32_t height,
                              uint64_t alreadyGeneratedCoins,
                              const CryptoNote::AccountPublicAddress &minerAddress, CryptoNote::Transaction &tx,
                              uint64_t fee, CryptoNote::KeyPair *pTxKey = 0);
