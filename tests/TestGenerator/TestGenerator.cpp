                              ...
                              std::vector<size_t> &blockSizes, uint64_t alreadyGeneratedCoins)
{
    ...
    m_blocksInfo[blk.getBlockHash()] = BlockInfo(blk.getBlock().previousBlockHash,
                                                 alreadyGeneratedCoins + emissionChange, blockSize);
}

bool
test_generator::constructBlock(CryptoNote::BlockTemplate &blk, uint32_t height, const Crypto::Hash &previousBlockHash,
                               const CryptoNote::AccountBase &minerAcc, uint64_t timestamp,
                               ...
                               std::vector<size_t> &blockSizes, const std::list<CryptoNote::Transaction> &txList)
{
    ...
    while (true)
    {
        if (!m_currency.constructMinerTx(blk.majorVersion, height, Common::medianValue(blockSizes),
                                         alreadyGeneratedCoins, targetBlockSize,
                                         totalFee, minerAcc.getAccountKeys().address, blk.baseTransaction,
                                         BinaryArray(), 10))
        {
            return false;
        }
...
bool test_generator::constructBlock(CryptoNote::BlockTemplate &blk, const CryptoNote::AccountBase &minerAcc,
                                    uint64_t timestamp)
{
                                    ...
                                    const std::list<CryptoNote::Transaction> &txList/* = std::list<CryptoNote::Transaction>()*/)
{
    ...
    return constructBlock(blk, height, previousBlockHash, minerAcc, timestamp, alreadyGeneratedCoins, blockSizes,
                          txList);
}

bool
test_generator::constructBlockManually(BlockTemplate &blk, const BlockTemplate &prevBlock, const AccountBase &minerAcc,
                                       int actualParams/* = bf_none*/, uint8_t majorVer/* = 0*/,
                                       uint8_t minorVer/* = 0*/, uint64_t timestamp/* = 0*/,
                                       const Crypto::Hash &previousBlockHash/* = Crypto::Hash()*/,
                                       const Difficulty &diffic/* = 1*/,
                                       ...
                                       size_t txsSizes/* = 0*/, uint64_t fee/* = 0*/)
{
    ...
    blk.timestamp = actualParams & bf_timestamp ? timestamp : prevBlock.timestamp +
                                                              m_currency.difficultyTarget(); // Keep difficulty unchanged
    ...
    } else
    {
        ...
        if (!m_currency.constructMinerTx(blk.majorVersion, height, Common::medianValue(blockSizes),
                                         alreadyGeneratedCoins, currentBlockSize, 0,
...
bool
test_generator::constructBlockManuallyTx(CryptoNote::BlockTemplate &blk, const CryptoNote::BlockTemplate &prevBlock,
                                         const CryptoNote::AccountBase &minerAcc,
                                           ...
                                           const std::list<CryptoNote::Transaction> &txList/* = std::list<CryptoNote::Transaction>()*/)
{
    ...
    bool r = constructMinerTxBySize(m_currency, baseTransaction, defaultMajorVersion,
                                    cachedPrevBlock.getBlockIndex() + 1,
                                    getAlreadyGeneratedCoins(cachedPrevBlock.getBlockHash()),
                                    minerAccount.getAccountKeys().address, blockSizes, 2 * median - txsSize, 2 * median,
                                    totalFee);
    if (!r)
    {
        return false;
    ...
    return constructBlockManually(blk, blkPrev, minerAccount,
                                  test_generator::bf_miner_tx | test_generator::bf_tx_hashes,
...
bool constructMinerTxManually(const CryptoNote::Currency &currency, uint8_t blockMajorVersion, uint32_t height,
                              uint64_t alreadyGeneratedCoins,
...
bool constructMinerTxBySize(const CryptoNote::Currency &currency, CryptoNote::Transaction &baseTransaction,
                            uint8_t blockMajorVersion, uint32_t height,
                            ...
                            uint64_t fee/* = 0*/)
{
    if (!currency.constructMinerTx(blockMajorVersion, height, Common::medianValue(blockSizes), alreadyGeneratedCoins,
                                   targetBlockSize,
