...
void NodeTest::readBlockchainInfo(INode &node, BlockchainInfo &bc)
{
    ...
    do
    {
        ...
        node.queryBlocks(std::vector<Crypto::Hash>(history.rbegin(), history.rend()), timestamp, blocks, startHeight,
                         cb.callback());
...
TEST_F(NodeTest, queryBlocks)
{
    ...
    auto startBlockIter = std::find_if(blocks.begin(), blocks.end(),
                                       [](const BlockShortEntry &e)
                                       { return e.hasBlock; });
