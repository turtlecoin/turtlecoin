...
public:
    ...
    MultiVersionTest(const CryptoNote::Currency &currency, System::Dispatcher &d,
                     const Tests::Common::BaseFunctionalTestsConfig &config, Logging::ILogger &log) :
            BaseFunctionalTests(currency, d, config), m_config(config), m_nodeCount(config.daemons.size()),
            logger(log, "MultiVersion")
    {}
        ...
        for (size_t i = 0; i < m_nodeCount; ++i)
        {
            ...
            for (size_t wi = 0; wi < m_nodeCount; ++wi)
            {
                if (i != wi)
                {
                    ...
                    logger(INFO, BRIGHT_YELLOW) << "Sending from " << shortAddress(srcWallet.getAddress()) << " to "
                                                << shortAddress(transfer.address) << " amount = "
    ...
    void miningTest()
    {
        ...
        for (size_t i = 0; i < m_nodeCount; ++i)
        {
            for (size_t shift = 0; shift < m_nodeCount; ++shift)
            {
                ...
                auto res = m_observers[shift]->waitPendingBalanceChangeFor(
                        std::chrono::seconds(m_currency.difficultyTarget() * 5));
