...
public:
    ...
    void printWalletBalances()
    {
        for (auto &w: wallets)
        {
            ...
            logger(INFO) << "  " << currency.formatAmount(w->actualBalance()) << " actual / "
                         << currency.formatAmount(w->pendingBalance()) << " pending";
        }
    }
...
TEST_F(IntegrationTest, BlockPropagationSpeed)
{
...

    {
        ...
        for (size_t blockNumber = 0; blockNumber < BLOCKS_COUNT; ++blockNumber)
        {
            ...
            {
                ASSERT_TRUE(
                        remoteObserver.waitLastKnownBlockHeightUpdated(std::chrono::milliseconds(5000), remoteHeight));
            }
